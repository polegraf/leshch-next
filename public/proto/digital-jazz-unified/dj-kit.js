...
window.djTournamentCard=o=>{const esc=v=>String(v??'').replace(/[&<>"]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot;'}[c]));
 ...
 return `<article class="t-card ${o.stage.kind}"${o.id!==undefined?` data-tournament-id="${o.id}"`:''}${o.urgent?' data-urgent="true"':''}${o.attrs||''}><a class="t-card-link" href="${o.href}" aria-label="${esc(o.name)} — ${esc(o.stage.label)}"><div class="t-card-cover"><img src="${o.cover}" alt="" loading="lazy"${o.coverClass?` class="${o.coverClass}"`:''}><span class="aud-badge t-card-status">${tournamentStatus(o.stage.label,o.stage.tone)}</span></div><div class="t-card-body"><p class="caption">${esc(o.category)} · ${esc(o.format)}${o.count?` · ${o.count} ${o.count%10===1&&o.count%100!==11?'участник':o.count%10>=2&&o.count%10<=4&&(o.count%100<12||o.count%100>14)?'участника':'участников'}`:''}${o.organizer?' · '+esc(o.organizer):''}</p><div class="t-card-title"><h3>${esc(o.name)}</h3><span class="aud-forward" aria-hidden="true">→</span></div>${o.dates?`<p class="t-card-date">${esc(o.dates)}</p>`:''}${jury.length||o.prize?`<div class="t-card-facts"><dl>${jury.length?`<dt>Жюри</dt><dd>${jury.map(j=>esc(j.replace('Судья ',''))).join(', ')}</dd>`:''}${o.prize?`<dt>Приз</dt><dd>${esc(o.prize)}</dd>`:''}</dl>${jury.length?`<span class="t-card-faces">${jury.map(j=>djPortrait(j,false)).join('')}</span>`:''}</div>`:''}${o.role?`<p class="t-card-role">${o.role}</p>`:''}</div></a></article>`;};

/* v68: одна турнирная сетка для всех мест — колонки раундов в полный размер, листается вправо, не ужимается.
   o:{aria, columns:[{name, tone:'ready'|'pending'|'upcoming', matches:[{label, rows:[{name, score, won, lost, me, tag}]}]}]} */
window.djBracket=o=>{const st={ready:'Завершён',pending:'Текущий раунд',upcoming:'Впереди'};
 const row=r=>`<div class="djb-row${r.won?' won':''}${r.lost?' lost':''}${r.me?' me':''}"><span class="djb-name">${r.name}${r.tag||''}</span>${r.score!==undefined&&r.score!==null?`<b>${r.score}</b>`:''}</div>`;
 return `<div class="bracket-scroll djb-scroll" tabindex="0" aria-label="${o.aria||'Турнирная сетка'}"><div class="djb-cols">${o.columns.map((c,ci)=>`<div class="djb-col" data-bracket-status="${c.tone}" style="--k:${2**ci}"><h4>${c.name}</h4><p class="caption bracket-status">${tournamentStatus(c.status||st[c.tone],c.tone)}</p><div class="djb-slots">${c.matches.map(m=>`<div class="djb-slot"><div class="djb-match${m.rows.some(r=>r.me)?' has-me':''}"><span class="caption">${m.label}</span>${m.rows.map(row).join('')}</div></div>`).join('')}</div></div>`).join('')}</div></div>`;};
