...
const people = [
 {name:'Судья Вектор',city:'Москва',role:'Диджей',genre:'Хип-хоп',initials:'СВ'},
 {name:'Судья Спектр',city:'Москва',role:'Диджей',genre:'Электроника',initials:'СС'},
 {name:'Судья Контур',city:'Санкт-Петербург',role:'Композитор',genre:'Поп',initials:'СК'},
 {name:'Судья Пульс',city:'Москва',role:'Битмейкер',genre:'Хип-хоп',initials:'СП'},
 {name:'Судья Искра',city:'Санкт-Петербург',role:'Вокалист',genre:'Поп',initials:'СИ'},
 {name:'Артист Ритм',city:'Москва',role:'Диджей',genre:'Электроника',initials:'АР'}
];
const products=[
...
function tabs(items,current,action){return `<div class="seg" ${action==='type'?'role="group" aria-label="Кого ищем"':''}>${items.map(x=>`<button data-action="${action}" data-value="${x}" aria-pressed="${x===current}">${x}</button>`).join('')}</div>`;}
...
// v54: блок, раскрытый ниже видимой зоны (под навигацией), подтягивается в видимую часть экрана.
function revealPanel(panel){requestAnimationFrame(()=>{const r=panel.getBoundingClientRect();const nav=document.querySelector('.bottom-nav-inner');const limit=(nav?nav.getBoundingClientRect().top:innerHeight)-96;if(r.top<=limit)return;const reduce=matchMedia('(prefers-reduced-motion: reduce)').matches;scrollBy({top:r.top-Math.round(innerHeight*.3),behavior:reduce?'auto':'smooth'});});}
...
function productPreview(p){if(p.category==='Мерч')return `<div class="merch-preview ${p.art}" aria-hidden="true"><svg viewBox="0 0 200 150">${p.art.startsWith('shirt')?'<path class="garment" d="M65 20 35 35 15 67 47 83 57 65 57 135 143 135 143 65 153 83 185 67 165 35 135 20Q100 42 65 20Z"/>':'<path class="garment" d="M45 95Q40 28 105 25Q163 27 160 97Z"/><path class="garment" d="M45 95Q110 80 160 97L188 118Q103 141 30 114Z"/>'}<image class="merch-logo" href="${p.art.includes('white')||p.art.includes('lime')?'assets/logo-black.png':'assets/logo.webp'}" x="81" y="52" width="38" height="38" preserveAspectRatio="xMidYMid meet"/></svg><small>DIGITAL JAZZ</small></div>`;if(p.category==='Тайлы')return `<div class="tile-preview plan-preview"><small>ТАЙЛ</small><strong>${p.label}</strong></div>`;return p.category==='Темы'?`<div class="theme-preview look-${p.art}" aria-hidden="true"><div class="preview-cover"></div><div class="preview-person"><span class="preview-avatar"><img src="assets/logo-black.png" alt=""></span><span><b>Твой профиль</b><small>Музыка — это ты</small></span></div><div class="preview-tags"><i>Диджей</i><i>Продюсер</i></div><div class="preview-lines"><i></i><i></i></div></div>`:`<div class="tile-preview look-${p.art}" aria-hidden="true"><span class="tile-signal"><i></i><i></i><i></i><i></i><i></i></span><strong>${p.label}</strong><small>DIGITAL JAZZ</small></div>`;}
...
document.addEventListener('click',e=>{const b=e.target.closest('[data-action]');if(!b)return;const {action,value}=b.dataset;
 ...
 case'services':$('#services').setAttribute('aria-expanded','true');openSheet('Сервисы','<div class="menu-links"><a href="#tournaments">Турниры <span>↗︎</span></a><a href="#shop">Магазин <span>↗︎</span></a></div>');break;
...
document.addEventListener('toggle', e => {
  ...
  if (!card || e.target.tagName !== 'DETAILS' || e.target.parentElement !== card) return; // v57: вложенные раскрытия не раскрывают карточку
