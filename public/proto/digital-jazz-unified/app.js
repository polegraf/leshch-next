...
{id:'tile',name:'На сцене',category:'Тайлы',price:80,art:'stage',label:'LIVE'},
{id:'light',name:'В эфире',category:'Тайлы',price:60,art:'air',label:'ON AIR'},
{id:'studio',name:'В студии',category:'Тайлы',price:60,art:'studio',label:'REC'},
{id:'tour',name:'В туре',category:'Тайлы',price:80,art:'tour',label:'ON TOUR'}];
...
function renderSearch(){return `<div class="heading"><h1>Поиск</h1><a class="text-button" href="#discovery">Дискавери ↗</a></div>${tabs(['Создатели','Менеджмент','Компании'],state.type,'type')}<div class="search-row"><input id="query" class="input" type="search" aria-label="Имя или профессия" placeholder="Имя или профессия" value="${esc(state.query)}"></div>${filterCard()}${filterChips()}<div class="section-title"><h2>Результаты</h2><span class="caption" id="result-count">${results().length} профилей</span></div><div class="stack" id="results">${peopleCards()}</div>`;}
...
function renderShop(){return `<div class="heading"><h1>Магазин</h1><button class="icon shop-cart" data-action="cart" aria-label="Корзина: ${cartCount()} товаров"><svg viewBox="0 0 24 24"><path d="M3 4h2l2 12h11l3-9H6M9 20h.01M17 20h.01"/></svg>${cartCount()?`<span class="cart-count">${cartCount()}</span>`:''}</button></div><div class="shop-wallet"><span>0 <span class="caption">Джазиксов</span></span><button class="text-button" data-action="topup">Пополнить</button></div>${tabs(['Каталог','Мои покупки'],state.shopTab,'shop-tab')}${state.shopTab==='Каталог'?`<div class="shop-intro"><h2>Твой звук. Твой стиль.</h2><p class="secondary">Примерь новое оформление профиля.</p></div>${shopShelf('Темы','Темы профиля','Цвет и настроение всей страницы')}${shopShelf('Тайлы','Тайлы','Твой статус в оформлении профиля')}<p class="caption shop-demo">Демонстрационная коллекция. Покупки не списывают средства.</p>`:`<section class="shop-shelf"><div class="shelf-heading"><div><h2>Твоя коллекция</h2><p class="caption">Купленное оформление остаётся здесь</p></div></div><div class="shop-rail">${products.filter(p=>p.owned).map(shopProduct).join('')}</div></section>`}`;}
function showShopProduct(id,tryOn=false){const p=products.find(p=>p.id===id);if(!p)return;shopState.preview=tryOn?p.id:null;openSheet(tryOn?'Примерка':p.name,`<div class="shop-detail">${tryOn?`<div class="tryon-caption"><span class="dot lime"></span>Предпросмотр · изменения не сохранены</div>`:''}${productPreview(p)}<div><h3>${p.name}</h3><p class="secondary">${p.category==='Темы'?'Палитра и обложка твоего профиля.':'Декоративный статус для твоего профиля.'}</p></div><p class="caption">${p.owned?'Уже в твоей коллекции':p.price+' Джазиксов · покупка навсегда'}</p><div class="shop-detail-actions">${!tryOn?`<button class="secondary-button" data-action="try-product" data-value="${p.id}">Примерить</button>`:''}${p.owned?`<button class="primary" data-action="apply-product" data-value="${p.id}">Применить</button>`:`<button class="primary" data-action="add-cart" data-value="${p.id}">${state.cart[p.id]?'В корзине':'В корзину'}</button>`}</div><p class="caption">Примерка на демонстрационном профиле.</p></div>`);}
...
function showCart(){const entries=products.filter(p=>state.cart[p.id]);openSheet('Корзина',entries.length?`<div class="stack">${entries.map(p=>`<div class="card cart-item"><div class="info"><h3>${p.name}</h3><p class="secondary">${p.price} Джазиксов</p></div><button class="icon" data-action="remove-cart" data-value="${p.id}" aria-label="Убрать ${p.name}">×</button></div>`).join('')}</div><div class="section-title"><h2>Итого</h2><span>${entries.reduce((n,p)=>n+p.price*state.cart[p.id],0)} Дж.</span></div><p class="secondary">Баланс: 0 Джазиксов. Для покупки нужно пополнить баланс.</p><button class="primary full actions" data-action="topup">Пополнить баланс</button>`:`<div class="empty"><h2>Пока пусто</h2><p class="secondary">Выбери оформление для профиля или мерч.</p><button class="primary" data-action="back-catalog">Открыть каталог</button></div>`);}
document.addEventListener('input',e=>{if(e.target.id==='query'){state.query=e.target.value;$('#results').innerHTML=peopleCards();$('#result-count').textContent=results().length+' профилей';}if(e.target.closest('#tournament-form')&&e.target.name)state.draft[e.target.name]=e.target.value;});
