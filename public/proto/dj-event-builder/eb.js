...
function prodCard(d){const n=d.rows.filter(x=>(x.l||'').trim()).length,provs=[...(venue()?[venue().name]:[]),...list('vendor').map(x=>x.name)],auto=d.custom?'':provFor(CAT_ROLE[d.cat])?.name||'';
 return `<details class="card eb-rd eb-prod" data-rd="${d.id}"${openR.has(d.id)?' open':''}><summary><span class="eb-rd-s"><b>${esc(d.n||'Без названия')}</b></span><span class="eb-rd-src">${n?n+' '+plural(n,'позиция','позиции','позиций'):'пусто'} · ${esc(d.prov||auto||'нет подрядчика')}</span><svg class="eb-rd-chev" viewBox="0 0 24 24" aria-hidden="true"><path d="m6 9 6 6 6-6"/></svg></summary>
 <div class="eb-rd-b">
...
function sheetHTML(){if(!sheet)return '';const s=sheet;let h='';
 ...
 if(s.type==='findv'){const d=E.prod.find(x=>x.id===s.id),rows=d.rows.filter(r=>(r.l||'').trim()&&r.q),L=vFind(d);
  h=`<p class="caption eb-sh-cap">Подбор по райдеру</p><h2 id="eb-sh-t">${esc(d.n||'Свой райдер')}</h2>${rows.length?`<p class="secondary">${rows.length} ${plural(rows.length,'позиция','позиции','позиций')}: ${rows.map(r=>esc(r.l)+' × '+r.q).join(', ')}. Кто закрывает больше — выше, при равенстве — дешевле.</p>
  ${L.length?`<ul class="card eb-pick eb-vfind">${L.map(({v,m})=>{const inTeam=E.people.some(p=>p.name===v.name),on=d.prov===v.name,ok=m.full===m.n;return `<li><span class="eb-ava eb-ava-i" style="background:var(--dj-avatar-${avaColor(v.name)})" aria-hidden="true">${esc(v.name[0])}</span><span class="eb-pick-t"><b>${esc(v.name)}</b><small><em class="eb-cov${ok?' ok':''}">${m.full} из ${m.n}</em> · ≈ ${rub(m.cost)}${inTeam?' · уже в команде':''}</small>${m.miss.length||m.short.length?`<small class="eb-vmiss">${[m.miss.length?'нет: '+m.miss.map(esc).join(', '):'',m.short.length?'мало: '+m.short.map(esc).join(', '):''].filter(Boolean).join(' · ')}</small>`:'<small class="eb-vmiss ok">закрывает весь райдер</small>'}</span>${on?'<span class="eb-mini on">Выбран</span>':`<button type="button" class="eb-mini${ok?' hot':''}" data-cmd="vpick:${d.id}:${encodeURIComponent(v.name)}:${m.cost}">Выбрать</button>`}</li>`;}).join('')}</ul><p class="caption">Цена — оценка по прайсу склада. Выбранный подрядчик добавится в команду со суммой оценки, запрос уйдёт из «Запросов».</p>`:'<div class="card eb-empty"><p class="secondary">В каталоге никто не закрывает эти позиции. Выбери подрядчика вручную или добавь своего в «Команде».</p></div>'}`:'<div class="card eb-empty"><p class="secondary">Сначала заполни райдер — подбор идёт по его строкам.</p></div>'}`;}
...
document.addEventListener('toggle',e=>{const d=e.target;if(d.dataset&&d.dataset.rd)d.open?openR.add(d.dataset.rd):openR.delete(d.dataset.rd);},true);
