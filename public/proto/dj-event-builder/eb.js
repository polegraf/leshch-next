...
document.addEventListener('input',e=>{const t=e.target;t.closest?.('.eb-dtf')?.classList.toggle('is-empty',!t.value);
 if(t.dataset.bind){const [o,k]=target(t.dataset.bind);if(!o)return;o[k]=k==='date'||(k==='d'&&t.type==='date')?(t.value?new Date(t.value+'T00:00'):null):['fee','cap','price','a','paid','budget','guests'].includes(k)?num(t.value):t.value;
