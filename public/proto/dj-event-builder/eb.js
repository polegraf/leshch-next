...
function initEv(ev){ev.people.forEach(p=>{if(p.rider&&!p.rider.rows){p.rider.rows=toRows(p.rider.t||[]);delete p.rider.t;}if(p.rider)p.rider.prov=p.rider.prov||{};});
 ...
 ev.codes=ev.codes||[];ev.formula=ev.formula||{on:false,fill:70,margin:20};
...
(()=>{const d=EVENTS.find(x=>x.id==='demo'),s=EVENTS.find(x=>x.id==='sale');if(d){d.codes=[{id:uid(),code:'JAZZ15',pct:15,limit:30}];const a=d.people.find(p=>p.group==='artist'&&p.rider);if(a)a.rider.rows.push({id:uid(),k:'strobe',l:'Стробоскоп',q:2,cat:'Свет'},{id:uid(),k:'haze',l:'Дым-машина',q:1,cat:'Свет'},{id:uid(),k:'riser',l:'Подиум для диджея',q:1,cat:'Сцена'},{id:uid(),k:'backdrop',l:'Бэкдроп с логотипом Digital Jazz',q:1,cat:'Декорации'});}
...
function autoProv(row){const d=DICT.find(x=>x[0]===row.k),v=venue();if(d&&d[3]==='Площадка')return v?.name||'';if(d&&provFor(d[3]))return provFor(d[3]).name;return provFor(CAT_ROLE[row.cat])?.name||'';}
...
function riderCard(p){const r=p.rider,rows=r?r.rows:[],cat=riderCat[p.id]||'Звук',cnt=k=>rows.filter(x=>x.cat===k&&(x.l||'').trim()).length,inCat=rows.filter(x=>x.cat===cat);
 const provs=[...(venue()?[venue().name]:[]),...list('vendor').map(x=>x.name),'Привозит артист'],auto=provFor(CAT_ROLE[cat])?.name,cur=r&&(r.prov||{})[cat]||'';
 ...
 <div class="eb-rd-an"><p class="eb-gt">Анализатор</p><textarea class="input eb-ta sm" rows="3" data-rdtext="${p.id}" placeholder="Вставь райдер из письма или PDF — разложим на звук, свет, сцену, декорации и бытовое"></textarea><button type="button" class="secondary-button eb-btn" data-cmd="rdparse:${p.id}">Разобрать</button></div>
 ...
 <div class="eb-chips wrap eb-rd-cats">${CATS.map(k=>`<button type="button" class="chip" data-rdcat="${p.id}:${k}" aria-pressed="${cat===k}">${k}${cnt(k)?`<em>${cnt(k)}</em>`:''}</button>`).join('')}</div>
 <label class="eb-rd-prov"><span class="caption">Подрядчик</span><select class="input" data-rdprov="${p.id}:${cat}"><option value="">${auto?'Авто: '+esc(auto):'Нет подрядчика'}</option>${provs.map(o=>`<option${o===cur?' selected':''}>${esc(o)}</option>`).join('')}</select></label>
 ${inCat.length?`<ul class="eb-rd-rows">${inCat.map(x=>`<li><input class="input" data-bind="rr:${x.id}.l" value="${esc(x.l)}" placeholder="Что нужно" aria-label="Позиция"><input class="input eb-q" inputmode="numeric" data-bind="rr:${x.id}.q" data-rerender value="${x.q||''}" aria-label="Сколько"><button type="button" class="eb-x" data-cmd="rddel:${p.id}:${x.id}" aria-label="Удалить строку">×</button></li>`).join('')}</ul>`:`<p class="caption">${cat}: пока пусто.</p>`}
...
function requests(){const A=list('artist'),N=needs(),H=hospNeeds(),R=reqs(),got=A.filter(p=>p.rider).length,noProv=N.filter(n=>!n.prov).length+H.filter(h=>!h.prov).length;
 ...
 <p class="eb-gt">Техника<span>${N.length}</span></p><ul class="card eb-needs">${N.sort((a,b)=>CATS.indexOf(a.cat)-CATS.indexOf(b.cat)).map(n=>`<li><button type="button" data-cmd="need:${n.key}"><span class="eb-need-t"><b>${esc(n.label)} <em>× ${n.qty}</em></b><small>${n.cat} · ${Object.keys(n.by).length>1?Object.entries(n.by).map(([sc,q])=>`${esc(sc)} ${q}`).join(' + ')+' · ':''}${n.src.join(', ')}${n.fromVenue&&n.prov!==venue()?.name?` · ${n.fromVenue} есть на площадке`:''}</small></span>${chip(n)}</button></li>`).join('')}</ul>
...
document.addEventListener('click',e=>{const t=e.target;
 ...
 switch(cmd){
  ...
  case 'rdrow':{const p=P(a),c=riderCat[a]||'Звук';ensureRider(p).rows.push({id:uid(),k:'',l:'',q:1,cat:c});openR.add(a);render();const L=document.querySelectorAll(`[data-rd="${a}"] .eb-rd-rows:not(.h) li`);L[L.length-1]?.querySelector('input')?.focus();return;}
  ...
  case 'rdparse':{const p=P(a),tx=document.querySelector(`[data-rdtext="${a}"]`)?.value||'';if(!tx.trim()){say('Вставь текст райдера — разберём по строкам');return;}const res=parseRider(tx);mergeParsed(p,res);openR.add(a);const by=CATS.map(c=>[c,res.tech.filter(x=>(KEY_CAT[x[0]]||'Звук')===c).length]).filter(x=>x[1]);if(by.length)riderCat[a]=by[0][0];render();say(`${p.name}: ${by.map(x=>x[0].toLowerCase()+' '+x[1]).join(', ')||'техники нет'}${res.hosp.length?', бытовое '+res.hosp.length:''}${res.notes.length+res.unknown.length?', в заметки '+(res.notes.length+res.unknown.length):''}`);return;}
