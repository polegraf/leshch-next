...
function bLineup(){const multi=E.scenes.length>1,cl=clashes();
 ...
 ${L.length?`<ul class="card eb-edit eb-lu">${L.map(p=>`<li>${ava(p,'sm')}<span class="eb-edit-n">${esc(p.name)}</span><label><span class="visually-hidden">Сет ${esc(p.name)}</span><input class="input eb-t" type="time" data-bind="p:${p.id}.set" value="${p.set||''}"></label><label><span class="visually-hidden">Гонорар ${esc(p.name)}</span><input class="input eb-m" inputmode="numeric" data-bind="p:${p.id}.fee" value="${p.fee||''}" placeholder="₽"></label><button type="button" class="eb-x" data-cmd="remove:${p.id}" aria-label="Убрать">×</button></li><li class="eb-rd-li">${riderCard(p)}</li>`).join('')}</ul>`:''}
...
function riderCard(p){const r=p.rider,rows=r?r.rows:[];
 const n=rows.filter(x=>(x.l||'').trim()).length,hh=r?r.h.filter(x=>x.trim()).length:0;
 const sum=r?`${n} ${plural(n,'позиция','позиции','позиций')}${hh?' · '+hh+' быт.':''}`:p.riderAsked?'запрошен '+p.riderAsked:'нет райдера';
 return `<details class="eb-rd" data-rd="${p.id}"${openR.has(p.id)?' open':''}><summary><span class="eb-rd-l">Райдер</span><span class="eb-rd-s${r?'':' eb-hot'}">${sum}</span>${r?`<span class="eb-rd-src">${RSRC[r.src]||''}</span>`:''}<svg class="eb-rd-chev" viewBox="0 0 24 24" aria-hidden="true"><path d="m6 9 6 6 6-6"/></svg></summary>
 <div class="eb-rd-b">
 ${r?'':`<div class="eb-rd-ask"><button type="button" class="eb-mini hot" data-cmd="askrider:${p.id}">${p.riderAsked?'Напомнить':'Запросить у артиста'}</button><span class="caption">или вставь текст ниже, или заполни таблицу</span></div>`}
 ...
 <p class="caption">Кто обеспечивает технику артиста — в «Запросах».</p>
 </div></details>`;}
const anBlock=(attr,id,cmd,ph)=>`<div class="eb-rd-an"><p class="eb-gt">Анализатор</p><textarea class="input eb-ta sm" rows="3" data-${attr}="${id}" placeholder="${ph}"></textarea><button type="button" class="secondary-button eb-btn" data-cmd="${cmd}:${id}">Разобрать</button></div>`;
...
document.addEventListener('input',e=>{const t=e.target;t.closest?.('.eb-dtf')?.classList.toggle('is-empty',!t.value);
 if(t.dataset.bind){const [o,k]=target(t.dataset.bind);if(!o)return;o[k]=k==='date'||(k==='d'&&t.type==='date')?(t.value?new Date(t.value+'T00:00'):null):['fee','cap','price','a','paid','budget','guests','qty','q','pct','limit','fill','margin'].includes(k)?num(t.value):k==='k'?(parseFloat(t.value.replace(',','.'))||0):k==='code'?t.value.toUpperCase().replace(/\s+/g,''):t.value;
