...
function prodCard(d){const n=d.rows.filter(x=>(x.l||'').trim()).length,provs=[...(venue()?[venue().name]:[]),...list('vendor').map(x=>x.name)],auto=d.custom?'':provFor(CAT_ROLE[d.cat])?.name||'';
 ...
 <label class="eb-rd-prov"><span class="caption">Подрядчик</span><select class="input" data-prodprov="${d.id}"><option value="">${auto?'Авто: '+esc(auto):'Не выбран'}</option>${provs.map(o=>`<option${o===d.prov?' selected':''}>${esc(o)}</option>`).join('')}</select></label>
