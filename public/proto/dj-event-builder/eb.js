...
const CAT={
 ...
 venue:[{name:'Цех 01',role:'Площадка',cap:350,fee:60000,city:'Москва',busy:['2026-10-17','2026-10-31','2026-11-07']},{name:'16 Тонн',role:'Клуб',cap:300,fee:70000,city:'Москва',busy:['2026-09-26','2026-10-03','2026-10-24','2026-11-14']},{name:'Atlas Club',role:'Клуб',cap:600,fee:120000,city:'Москва',busy:['2026-10-10','2026-10-24','2026-11-21']},{name:'ARCHILOFT',role:'Лофт с бассейном',cap:400,fee:150000,city:'Москва',busy:['2026-10-23','2026-11-14'],outdoor:true},{name:'Aglomerat',role:'Площадка',cap:1500,fee:280000,city:'Москва',busy:['2026-10-25','2026-11-01']},{name:'90Floor',role:'Руфтоп',cap:250,fee:90000,city:'Москва',busy:['2026-10-24'],outdoor:true},{name:'Art House',role:'Клуб',cap:200,fee:40000,city:'Раменское',busy:[]},{name:'ARCHILOFT Малый зал',role:'Лофт',cap:180,fee:55000,city:'Москва',busy:['2026-10-31']},{name:'Завод Сцена',role:'Площадка',cap:900,fee:190000,city:'Москва',busy:['2026-10-17','2026-10-24']}]};
...
DICT.push(['strobe','Стробоскоп',/строб/i,'Сцена и свет'],['haze','Дым-машина',/дым|хейз|haze/i,'Сцена и свет'],['wash','Заливной свет',/заливн|wash|прожектор/i,'Сцена и свет'],['riser','Подиум для диджея',/подиум|райзер|riser/i,'Сцена и свет'],['backdrop','Бэкдроп',/бэкдроп|задник|баннер|backdrop/i,'Монтаж сцены'],['deco','Декор сцены',/декор/i,'Монтаж сцены']);
/* четыре плашки техники в райдере; подрядчик по умолчанию — по роли */
const CATS=['Звук','Свет','Сцена','Декорации'];
/* склад подрядчиков: [что умеет найти в строке райдера, сколько есть, ₽ за единицу] — по нему «Найти» собирает шорт-лист */
const VKIT={'Звуковой склад':[[/портал|сателлит|l-acoustics/i,8,6000],[/саб/i,8,5000],[/пульт foh|звуков\S* пульт|^пульт/i,2,12000],[/мониторн\S* лини/i,4,3000],[/монитор/i,10,2500],[/радиомикрофон/i,4,2000],[/sm58|sm57|микрофон/i,20,500],[/\bdi\b|di-box/i,10,400]],
 'Саунд Про':[[/портал|сателлит|l-acoustics/i,4,7000],[/саб/i,4,6000],[/пульт foh|звуков\S* пульт|^пульт/i,1,15000],[/монитор/i,6,3000],[/радиомикрофон/i,6,1800]],
 'Эхо Рентал':[[/портал|сателлит|l-acoustics/i,12,5000],[/саб/i,12,4500],[/пульт foh|звуков\S* пульт|^пульт/i,3,10000],[/мониторн\S* лини/i,2,3500],[/радиомикрофон/i,2,2500],[/монитор/i,8,2000]],
 'Сценический прокат':[[/голов/i,12,3500],[/строб/i,6,1500],[/дым|хейз/i,2,2500],[/световой пульт/i,2,9000],[/заливн|прожектор/i,24,800],[/ферм/i,20,1200],[/подиум/i,4,3000]],
 'Лайтбокс':[[/голов/i,6,3000],[/строб/i,4,1200],[/дым|хейз/i,1,2000],[/световой пульт/i,1,8000]],
 'Конструкция':[[/ферм/i,40,1000],[/бэкдроп|задник/i,1,15000],[/подиум/i,6,2500]],
 'Декор Лаб':[[/бэкдроп|задник|баннер/i,3,12000],[/неон/i,4,9000],[/ферм/i,6,1500],[/декор/i,10,5000]],
 'Видеостена':[[/led|экран/i,2,60000],[/медиасервер/i,2,15000]]};
function vMatch(name,rows){const K=VKIT[name]||[];let cost=0,full=0;const miss=[],short=[];rows.forEach(r=>{const k=K.find(x=>x[0].test(r.l));if(!k){miss.push(r.l);return;}const q=Math.min(k[1],r.q);cost+=q*k[2];if(k[1]>=r.q)full++;else short.push(`${r.l} ${k[1]} из ${r.q}`);});return {full,part:short.length,miss,short,cost,n:rows.length};}
function vCalc(name,rows){const K=VKIT[name]||[];return rows.map(r=>{const k=K.find(x=>x[0].test(r.l));if(!k)return {l:r.l,q:r.q,have:0,pr:0,sum:0,st:'none'};const have=Math.min(k[1],r.q);return {l:r.l,q:r.q,have,pr:k[2],sum:have*k[2],st:k[1]>=r.q?'ok':'short'};});}
const quoteSum=(name,est)=>Math.round(est*(.86+[...name].reduce((a,c)=>a+c.charCodeAt(0),0)%22/100)/100)*100;
const Q_ST={sent:['ждём расчёт','wait'],quoted:['расчёт','ok'],accepted:['выбран','ok'],closed:['не выбран','']};
function quotesBlock(d){const Q=d.quotes||[];if(!Q.length)return '';const best=Q.filter(q=>q.st==='quoted').sort((a,b)=>a.sum-b.sum)[0];
 return `<p class="eb-gt">Запросы расчёта<span>${Q.filter(q=>q.st==='quoted').length} из ${Q.length} ответили</span></p><ul class="eb-quotes">${Q.map(q=>{const [l,c]=Q_ST[q.st];return `<li class="${q.st}"><span class="eb-need-t"><b>${esc(q.name)}</b><small><em class="eb-rq ${c}">${l}${q.sum&&q.st!=='sent'?' '+rub(q.sum):''}</em>${q.st==='quoted'?` · оценка была ${rub(q.est)}`:q.st==='sent'?' · отправлен '+q.at:''}${best&&q===best&&Q.filter(x=>x.st==='quoted').length>1?' · <b class="eb-ok">дешевле всех</b>':''}</small></span>${q.st==='quoted'?`<button type="button" class="eb-mini hot" data-cmd="qacc:${d.id}:${encodeURIComponent(q.name)}">Принять</button>`:q.st==='sent'?`<button type="button" class="eb-mini" data-cmd="qnud:${encodeURIComponent(q.name)}">Напомнить</button>`:''}</li>`;}).join('')}</ul>`;}
function vFind(d){const rows=d.rows.filter(r=>(r.l||'').trim()&&r.q);return CAT.vendor.filter(v=>VKIT[v.name]).map(v=>({v,m:vMatch(v.name,rows)})).filter(x=>x.m.full+x.m.part>0).sort((a,b)=>b.m.full-a.m.full||b.m.part-a.m.part||a.m.cost-b.m.cost);}
const KEY_CAT={strobe:'Свет',haze:'Свет',wash:'Свет',riser:'Сцена',drumkit:'Сцена',table:'Сцена','guitar-amp':'Сцена',power:'Сцена',backdrop:'Декорации',deco:'Декорации'};
const CAT_ROLE={Звук:'Звук',Свет:'Сцена и свет',Сцена:'Сцена и свет',Декорации:'Монтаж сцены'};
const RSRC={profile:'из профиля',text:'из письма',file:'загружен артистом',manual:'заполнен вручную'};
const toRows=t=>t.map(([k,l,q])=>({id:Math.random().toString(36).slice(2,9),k,l,q,cat:KEY_CAT[k]||'Звук'}));
...
function postMock(p,cls=''){if(p.own)return `<div class="eb-mock own ${cls}"><span>Свой макет</span><small>${esc(p.own)}</small></div>`;
 ...
 const body={poster:`${bg}<div class="eb-mock-c"><span>Digital Jazz</span><b>${esc(E.name)}</b><em>${date} · ${esc(venue()?.name||E.city)}</em></div>`,
  ...
  sale:`${bg}<div class="eb-mock-c"><span>Билеты в продаже</span><b>${minPrice()?'от '+rub(minPrice()):'Регистрация'}</b><em>${esc(E.name)} · ${date}</em></div>`,
...
function captionFor(p){const k=postKind(p),A=list('artist').map(a=>a.name).join(', ');return {poster:`${E.name} — ${E.date?dm(E.date):'скоро'}, ${venue()?.name||E.city}. Лайн-ап и билеты — по ссылке в профиле.`,lineup:`Кто играет на ${E.name}: ${A}. ${E.date?dm(E.date):''}.`,times:`Сет-тайм ${E.name}: ${sets().map(a=>a.set+' '+a.name).join(', ')}. Двери в ${E.start}.`,sale:`Билеты на ${E.name} уже в продаже${minPrice()?' — от '+rub(minPrice()):''}. Ранние — ограничены.`,tickets:`Последние билеты на ${E.name}. Дальше — только на входе, если останутся.`,today:`Сегодня! ${E.name}, двери в ${E.start}. Увидимся.`,after:`Спасибо, что были с нами на ${E.name}. Ищите себя на фото.`}[k];}
function pressDraft(){const A=list('artist').map(p=>p.name);return `${E.name} — ${E.type.toLowerCase()} Digital Jazz${E.date?' '+dm(E.date):''}${venue()?' в '+venue().name:''}, ${E.city}. Начало в ${E.start}.\n\nВ лайн-апе: ${A.join(', ')||'объявим скоро'}.\n\n${E.ticketMode==='sale'&&minPrice()?'Билеты от '+rub(minPrice())+' — на digitaljazz.ru/e/'+E.id+'.':'Вход свободный по регистрации.'}\n\nКонтакт для прессы: Digital Jazz, press@digitaljazz.ru`;}
...
/* ---------------- события ---------------- */
const riderFor=n=>{const r=PROFILE_RIDERS[n];return r&&n!=='Рифф'?{src:'profile',f:r.f,rows:toRows(r.t),h:[...r.h],n:r.n,prov:{}}:null;};
function demo(){const E={reqOver:{},id:'demo',name:'Digital Jazz Night',type:'Фестиваль',date:new Date(2026,9,24),start:'18:00',end:'23:30',city:'Москва',cover:'jazz-band',announce:new Date(2026,9,3),ticketMode:'reg',price:1800,reserve:.1,people:[],costs:[],incomes:[],stages:[],slots:{crew:[...ROLES_BIG.crew],vendor:[...ROLES_BIG.vendor]}};
 ...
 return E;}
function blank(){return {guests:0,scenes:[{id:'main',n:'Главная сцена'}],assets:{},reqSt:{},reqOver:{},channels:[],posts:[],assets:{},sales:[],id:uid(),name:'',type:'Вечеринка',date:null,start:'23:00',end:'06:00',city:'Москва',cover:null,announce:null,ticketMode:'sale',price:0,reserve:.1,people:[],costs:[],incomes:[],stages:[],slots:{crew:[...ROLES.crew],vendor:[...ROLES.vendor]},fresh:true};}
...
EVENTS.forEach(ev=>{if(!ev.guests)ev.guests=(ev.people.find(p=>p.group==='venue')||{}).cap||0;});
/* билеты: типы с квотой и ценой; E.price — средняя цена с учётом скидок (для окупаемости) */
function initEv(ev){ev.people.forEach(p=>{if(p.rider&&!p.rider.rows){p.rider.rows=toRows(p.rider.t||[]);delete p.rider.t;}if(p.rider)p.rider.prov=p.rider.prov||{};});
 if(!ev.tickets){const c=(ev.people.find(p=>p.group==='venue')||{}).cap||0,pr=ev.price||0,a=Math.round(c*.2),b=Math.round(c*.7);
  ev.tickets=pr?[{id:uid(),n:'Ранний',qty:a,price:Math.round(pr*.8/100)*100,k:.8,desc:'Первые 20% мест — до объявления полного лайн-апа'},{id:uid(),n:'Стандарт',qty:b,price:pr,k:1,desc:'Основная продажа'},{id:uid(),n:'VIP',qty:c-a-b,price:pr*2,k:2,desc:'Зона у сцены и вход без очереди'}]:[{id:uid(),n:'Входной',qty:c,price:0,k:1,desc:''}];}
 ev.codes=ev.codes||[];ev.prod=ev.prod||[['Звук','Звук'],['Свет','Свет'],['Декор','Декорации']].map(([n,c])=>({id:uid(),n,cat:c,rows:[],note:'',prov:'',custom:false}));ev.formula=ev.formula||{on:false,fill:70,margin:20};
 delete ev.price;Object.defineProperty(ev,'price',{configurable:true,enumerable:false,get(){const s=E;E=this;try{return avgPrice();}finally{E=s;}}});return ev;}
EVENTS.forEach(initEv);
/* подрядчики только из каталога — в демо-события не входят */
CAT.vendor.push(...[['Саунд Про','Звук'],['Эхо Рентал','Звук'],['Лайтбокс','Сцена и свет'],['Декор Лаб','Монтаж сцены'],['Видеостена','Видео']].map(([n,r])=>({name:n,role:r,city:'Москва'})));
(()=>{const d=EVENTS.find(x=>x.id==='demo'),s=EVENTS.find(x=>x.id==='sale');if(d){d.codes=[{id:uid(),code:'JAZZ15',pct:15,limit:30}];const a=d.people.find(p=>p.group==='artist'&&p.rider);const P3=n=>d.prod.find(x=>x.n===n),row=(l,q,cat)=>({id:uid(),k:'',l,q,cat});P3('Звук').rows.push(row('Порталы и сабы на 350 гостей',1,'Звук'),row('Пульт FOH',1,'Звук'),row('Радиомикрофон для ведущего',2,'Звук'));P3('Свет').rows.push(row('Световые головы',8,'Свет'),row('Стробоскоп',2,'Свет'),row('Дым-машина',1,'Свет'),row('Световой пульт',1,'Свет'));P3('Свет').note='Свет на обе сцены, оператор с 17:00';P3('Декор').rows.push(row('Бэкдроп с логотипом Digital Jazz',1,'Декорации'),row('Неоновая вывеска',1,'Декорации'));d.prod.push({id:uid(),n:'Видео',cat:'',rows:[row('LED-экран за сценой 4×2 м',1,''),row('Медиасервер',1,'')],note:'',prov:'',custom:true});}
 if(s)s.codes=[{id:uid(),code:'EARLY10',pct:10,limit:50},{id:uid(),code:'FRIENDS',pct:20,limit:20}];})();
...
function autoProv(row){if(!row.cat)return '';const d=DICT.find(x=>x[0]===row.k),v=venue();if(d&&d[3]==='Площадка')return v?.name||'';if(d&&provFor(d[3]))return provFor(d[3]).name;return provFor(CAT_ROLE[row.cat])?.name||'';}
function needs(){const map=new Map();list('artist').filter(p=>p.rider).forEach(p=>{const sc=sceneOf(p.scene);p.rider.rows.forEach(r=>{const l=(r.l||'').trim();if(!l||!r.q)return;const key=r.k||'x-'+l.toLowerCase().replace(/[^a-zа-яё0-9]+/gi,'-');const o=map.get(key)||{key,label:l,cat:r.cat,qty:0,src:[],by:{},pov:'',row:r};o.by[sc.n]=Math.max(o.by[sc.n]||0,r.q);if(!o.src.includes(p.name))o.src.push(p.name);const po=(p.rider.prov||{})[r.cat];if(po&&!o.pov)o.pov=po;map.set(key,o);});});
 (E.prod||[]).forEach(d=>d.rows.forEach(r=>{const l=(r.l||'').trim();if(!l||!r.q)return;const key='p-'+d.id+'-'+l.toLowerCase().replace(/[^a-zа-яё0-9]+/gi,'-');const o=map.get(key)||{key,label:l,cat:d.n,qty:0,src:[],by:{},pov:d.prov,row:{...r,cat:d.cat},prod:d};o.by['Событие']=(o.by['Событие']||0)+r.q;const sl='райдер «'+(d.n||'свой')+'»';if(!o.src.includes(sl))o.src.push(sl);map.set(key,o);}));
 map.forEach(o=>{o.qty=Object.values(o.by).reduce((a,x)=>a+x,0);});
 const v=venue(),kit=v&&VENUE_KIT[v.name]||{};
 return [...map.values()].map(n=>{const ov=(E.reqOver||{})[n.key]||{};const qty=ov.qty??n.qty;const d=DICT.find(x=>x[0]===n.key);const fromVenue=Math.min(qty,kit[n.key]||0);
  let prov=ov.prov||n.pov;if(!prov)prov=d&&d[3]==='Площадка'&&v?v.name:fromVenue>=qty&&v?v.name:autoProv(n.row);return {...n,qty,fromVenue:prov===v?.name?qty:fromVenue,prov};});}
function hospNeeds(){const out=[];list('artist').filter(p=>p.rider).forEach(p=>p.rider.h.filter(x=>x.trim()).forEach(line=>{const h=HOSP.find(x=>x[0].test(line));const cat=h?h[1]:'Площадка';const prov=cat==='Площадка'?venue()?.name||'':provFor(cat)?.name||'';out.push({line,who:p.name,cat,prov});}));return out;}
...
const tixQty=()=>(E.tickets||[]).reduce((a,t)=>a+(t.qty||0),0);
function fW(){return (E.tickets||[]).filter(t=>t.qty>0).reduce((a,t)=>a+t.qty*(t.k||1),0)||cap();}
function fBase(){const F=E.formula,w=fW();if(!w)return 0;return Math.max(0,cost().total-income(false))*(1+(F.margin||0)/100)/(w*(F.fill||100)/100);}
const tPrice=t=>E.formula&&E.formula.on?Math.ceil(fBase()*(t.k||1)/100)*100:(t.price||0);
function rawAvg(){const T=(E.tickets||[]).filter(t=>t.qty>0&&tPrice(t)>0),q=T.reduce((a,t)=>a+t.qty,0);if(!q){const p=(E.tickets||[]).map(tPrice).filter(Boolean);return p.length?Math.min(...p):0;}return T.reduce((a,t)=>a+t.qty*tPrice(t),0)/q;}
const codeUse=c=>c.limit||Math.round((tixQty()||cap())*.1);
const discLoss=()=>{const r=rawAvg();return (E.codes||[]).reduce((a,c)=>a+codeUse(c)*r*(c.pct||0)/100,0);};
function avgPrice(){if(!E.tickets)return 0;const r=rawAvg(),q=tixQty()||cap();return r?Math.max(0,Math.round(r-(q?discLoss()/q:0))):0;}
const minPrice=()=>{const p=(E.tickets||[]).map(tPrice).filter(Boolean);return p.length?Math.min(...p):0;};
function soldBy(){let s=soldTotal();return E.tickets.map(t=>{const x=Math.min(t.qty||0,s);s-=x;return x;});}
...
const route=()=>{const h=location.hash.slice(1)||'summary';const b=h.match(/^build\/(\d)$/);return b?{v:'build',step:+b[1]}:{v:['events','summary','place','lineup','people','req','tickets','promo','money','day'].includes(h)?h:'summary'};};
...
/* ================= БИЛДЕР ================= */
const STEPS=['Основа','Площадка','Лайн-ап','Команда','Запросы','Билеты','Деньги','Продвижение','Проверка'];
function stepDone(i){return [!!E.name&&!!E.date,!!venue(),list('artist').length>0&&list('artist').every(p=>p.rider),E.slots.crew.concat(E.slots.vendor).every(r=>E.people.some(p=>p.role===r))&&E.people.some(p=>p.group==='crew'),reqs().length>0&&reqs().every(r=>r.st!=='draft'),E.ticketMode==='reg'||E.tickets.length>0&&E.tickets.every(t=>t.qty>0&&tPrice(t)>0),cost().total>0&&(E.ticketMode==='reg'||breakEven(false)!==null&&(!cap()||breakEven(false)<=cap())),(E.channels||[]).length>0&&(E.posts||[]).length>0,false][i];}
function build(step){const monitor=!E.fresh;
 ...
 const body=[bBasic,bVenue,bLineup,bTeam,bRequests,tickets,bMoney,bPromo,bReview][step]();
 const foot=step<8?`<div class="eb-bfoot">${step?`<a class="secondary-button eb-btn" href="#build/${step-1}">Назад</a>`:'<span></span>'}<button type="button" class="primary eb-btn" data-cmd="next:${step}">${step===7?'К проверке':'Дальше'}</button></div>`:'';
 return head+`<div class="eb-bbody">${body}</div>`+foot;}

function bBasic(){return `${E.fresh?`<button type="button" class="card eb-ai" data-cmd="ai"><span class="eb-ai-i" aria-hidden="true">✦</span><span class="eb-need-t"><b>Собрать с ИИ-помощником</b><small>Опиши вечер словами — ИИ предложит дату, площадку, лайн-ап и бюджет</small></span></button>`:''}<label class="field"><span>Название</span><input class="input" data-bind="E.name" value="${esc(E.name)}" placeholder="Digital Jazz Night"></label>
 ...
 <div class="eb-two"><label class="field eb-dtf${E.date?'':' is-empty'}"><span>Дата</span><span class="eb-dtwrap"><input class="input" type="date" data-bind="E.date" data-rerender value="${iso(E.date)}"><em aria-hidden="true">Выбрать</em></span></label><label class="field"><span>Город</span><input class="input" data-bind="E.city" value="${esc(E.city)}"></label></div>
 ...
 ${E.date?dateCard():'<p class="caption">Нет даты — сохраним черновик, сроки появятся позже.</p>'}
 <label class="field"><span>Ожидаем гостей</span><input class="input" inputmode="numeric" data-bind="E.guests" data-rerender value="${E.guests||''}" placeholder="Например, 300"></label>
...
function bVenue(){const v=venue(),vx=v&&CAT.venue.find(x=>x.name===v.name),busy=vx&&E.date&&!vFree(vx,E.date);if(busy)return `${pcard(v)}<p class="eb-warn">${esc(v.name)} занята ${dm(E.date)}. Свободна: ${vNear(vx,E.date).map(z=>dm(z)).join(', ')||'нет рядом'}.</p><button type="button" class="primary eb-btn eb-full" data-cmd="pick:venue">Подобрать свободную</button><button type="button" class="secondary-button eb-btn eb-full" data-cmd="dateinfo">Сменить дату</button>`;
 ...
 :`<p class="secondary">Подбираем под ${E.guests?E.guests+' гостей':'число гостей'}${E.date?' и '+dm(E.date):''}. Вместимость площадки — потолок продаж.</p>${pickList('venue','')}<button type="button" class="text-button" data-cmd="manual:venue">Моей площадки нет в каталоге</button>`;}

function bLineup(){const multi=E.scenes.length>1,cl=clashes();
 return `<p class="secondary">${list('artist').length?'Открой артиста — внутри время сета, гонорар и райдер. Точка справа — статус: зелёная — всё готово, фиолетовая — в работе, розовая — нужно действие.':'Добавь артистов на сцену — время расставим подряд, поменяешь здесь же.'}</p>
 ...
 ${L.length?`<ul class="card eb-arts">${L.map(p=>`<li>${riderCard(p)}</li>`).join('')}</ul>`:''}
 ...
 <div class="eb-act-btns">${addBtn('Сцена','addscene')}<button type="button" class="text-button" data-cmd="manual:artist">Артиста нет в Digital Jazz</button></div>${prodSection()}`;}
/* райдер — прикрытая карточка в строке артиста: анализатор, свой текст, техника по плашкам с подрядчиком, бытовое */
const openR=new Set(),riderCat={};
/* артист в лайн-апе: закрыт — аватар, имя, время сета, маркер статуса; открыт — условия, следующий шаг, райдер */
function artMark(p){const ns=nextStep(p);return ns[0]==='Всё готово'?['ok','всё готово']:ns[3]==='hot'?['hot',ns[0]]:['work',ns[0]];}
function riderCard(p){const r=p.rider,rows=r?r.rows:[],[mk,mt]=artMark(p),ns=nextStep(p);
 const n=rows.filter(x=>(x.l||'').trim()).length,hh=r?r.h.filter(x=>x.trim()).length:0;
 return `<details class="eb-art" data-rd="${p.id}"${openR.has(p.id)?' open':''}><summary>${ava(p,'sm')}<span class="eb-art-n">${esc(p.name)}</span><span class="eb-art-t">${p.set||'—'}</span><i class="eb-mk ${mk}" role="img" aria-label="${esc(mt)}" title="${esc(mt)}"></i><svg class="eb-rd-chev" viewBox="0 0 24 24" aria-hidden="true"><path d="m6 9 6 6 6-6"/></svg></summary>
 <div class="eb-rd-b">
 <div class="eb-art-st"><span class="eb-mk ${mk}" aria-hidden="true"></span><span>${esc(mt)}</span>${ns[1]?`<button type="button" class="eb-mini${ns[3]?' hot':''}" data-cmd="${ns[2]}">${ns[1]}</button>`:''}</div>
 <div class="eb-art-f"><label class="field"><span>Начало сета</span><input class="input" type="time" data-bind="p:${p.id}.set" data-rerender value="${p.set||''}"></label><label class="field"><span>Минут</span><input class="input" inputmode="numeric" data-bind="p:${p.id}.len" data-rerender value="${p.len||''}"></label><label class="field"><span>Гонорар, ₽</span><input class="input" inputmode="numeric" data-bind="p:${p.id}.fee" value="${p.fee||''}"></label></div>
 <p class="eb-gt eb-art-h">Райдер<span>${r?`${n} ${plural(n,'позиция','позиции','позиций')}${hh?' · '+hh+' быт.':''} · ${RSRC[r.src]||''}`:p.riderAsked?'запрошен '+p.riderAsked:'<b class="eb-hot">нет</b>'}</span></p>
 ${r?'':`<div class="eb-rd-ask"><button type="button" class="eb-mini hot" data-cmd="askrider:${p.id}">${p.riderAsked?'Напомнить':'Запросить у артиста'}</button><span class="caption">или вставь текст, или заполни таблицу</span></div>`}
 ${anBlock('rdtext',p.id,'rdparse','Вставь райдер артиста из письма или PDF — разложим на технику и бытовое')}
 <label class="field eb-rd-own"><span>Свой текст</span><textarea class="input eb-ta sm" rows="2" data-rdnote="${p.id}" placeholder="Саундчек, что привозит артист, особые условия">${esc(r?.n||'')}</textarea></label>
 <p class="eb-gt">Техника<span>${n}</span></p>
 ${rows.length?rowsList(rows,'rddel:'+p.id):''}
 ${addBtn('Строка','rdrow:'+p.id)}
 <p class="eb-gt">Бытовое<span>${hh}</span></p>
 ${r&&r.h.length?`<ul class="eb-rd-rows h">${r.h.map((x,i)=>`<li><input class="input" data-bind="rh:${p.id}.${i}" value="${esc(x)}" placeholder="Вода, гримёрная, питание" aria-label="Бытовое"><button type="button" class="eb-x" data-cmd="rdhdel:${p.id}:${i}" aria-label="Удалить строку">×</button></li>`).join('')}</ul>`:''}
 ${addBtn('Строка','rdh:'+p.id)}
 <div class="eb-art-foot"><button type="button" class="text-button" data-open="${p.id}">Статусы и оплата</button><button type="button" class="text-button eb-hot" data-cmd="remove:${p.id}">Убрать из лайн-апа</button></div>
 </div></details>`;}
const IC_UP='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M12 16V4M7 9l5-5 5 5M5 20h14"/></svg>',IC_DOC='<svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 3h7l5 5v13H7z"/><path d="M14 3v5h5"/><text x="13" y="18" text-anchor="middle">PDF</text></svg>';
function filesOf(id){const p=E.people.find(x=>x.id===id);if(p)return p.rider?(p.rider.files=p.rider.files||(p.rider.src==='profile'?[{name:'Райдер_'+p.name+'.pdf',src:'profile'}]:[])):[];const d=(E.prod||[]).find(x=>x.id===id);return d?(d.files=d.files||[]):[];}
const fileChips=id=>{const F=filesOf(id);return F.length?`<ul class="eb-files">${F.map((f,i)=>`<li><button type="button" class="eb-file-o" data-cmd="fileopen:${id}:${i}">${IC_DOC}<span><b>${esc(f.name)}</b><small>${f.src==='profile'?'из профиля артиста':'загружен · разобран'}</small></span></button><button type="button" class="eb-x" data-cmd="filedel:${id}:${i}" aria-label="Убрать файл">×</button></li>`).join('')}</ul>`:'';};
const anBlock=(attr,id,cmd,ph)=>`<div class="eb-rd-an"><div class="eb-rd-anh"><p class="eb-gt">Анализатор</p><label class="eb-up">${IC_UP}<span>Загрузить PDF</span><input type="file" accept=".pdf,.doc,.docx,.txt,image/*" data-rdfile="${id}" data-parse="${cmd}"></label></div>${fileChips(id)}<textarea class="input eb-ta sm" rows="3" data-${attr}="${id}" placeholder="${ph} — или загрузи PDF"></textarea><button type="button" class="secondary-button eb-btn" data-cmd="${cmd}:${id}">Разобрать</button></div>`;
const rowsList=(rows,del)=>`<ul class="eb-rd-rows">${rows.map(x=>`<li><input class="input" data-bind="rr:${x.id}.l" value="${esc(x.l)}" placeholder="Что нужно" aria-label="Позиция"><input class="input eb-q" inputmode="numeric" data-bind="rr:${x.id}.q" data-rerender value="${x.q||''}" aria-label="Сколько"><button type="button" class="eb-x" data-cmd="${del}:${x.id}" aria-label="Удалить строку">×</button></li>`).join('')}</ul>`;
/* райдеры события — не привязаны к артистам: звук, свет, декор и свои */
const PROD_KIND={Звук:'Звук',Свет:'Свет',Декор:'Декорации'};
function prodCard(d){const n=d.rows.filter(x=>(x.l||'').trim()).length,provs=[...(venue()?[venue().name]:[]),...list('vendor').map(x=>x.name)],auto=d.custom?'':provFor(CAT_ROLE[d.cat])?.name||'';
 return `<details class="card eb-rd eb-prod" data-rd="${d.id}"${openR.has(d.id)?' open':''}><summary><span class="eb-rd-s"><b>${esc(d.n||'Без названия')}</b></span><span class="eb-rd-src">${n?n+' '+plural(n,'позиция','позиции','позиций'):'пусто'} · ${(d.quotes||[]).some(q=>q.st==='quoted')&&!d.prov?`<b class="eb-ok">${(d.quotes||[]).filter(q=>q.st==='quoted').length} расчёта</b>`:(d.quotes||[]).some(q=>q.st==='sent')&&!d.prov?'ждём расчёт':esc(d.prov||auto||'нет подрядчика')}</span><svg class="eb-rd-chev" viewBox="0 0 24 24" aria-hidden="true"><path d="m6 9 6 6 6-6"/></svg></summary>
 <div class="eb-rd-b">
 ${d.custom?`<div class="eb-tk-h"><input class="input eb-tk-n" data-bind="pr:${d.id}.n" data-rerender value="${esc(d.n)}" placeholder="Название: спецэффекты, видео, сцена…" aria-label="Название райдера"><button type="button" class="eb-x" data-cmd="delprod:${d.id}" aria-label="Удалить райдер">×</button></div>`:''}
 ${anBlock('prtext',d.id,'prodparse','Вставь ТЗ или райдер из письма — разложим по строкам')}
 <label class="field eb-rd-own"><span>Свой текст</span><textarea class="input eb-ta sm" rows="2" data-bind="pr:${d.id}.note" placeholder="Задача подрядчику своими словами">${esc(d.note||'')}</textarea></label>
 <div class="eb-rd-prov"><span class="caption">Подрядчик</span><select class="input" data-prodprov="${d.id}" aria-label="Подрядчик"><option value="">${auto?'Авто: '+esc(auto):'Не выбран'}</option>${provs.map(o=>`<option${o===d.prov?' selected':''}>${esc(o)}</option>`).join('')}</select><button type="button" class="eb-mini eb-find" data-cmd="findv:${d.id}"><svg viewBox="0 0 24 24" aria-hidden="true"><circle cx="11" cy="11" r="6.5"/><path d="m16 16 4 4"/></svg>Найти</button></div>
 ${quotesBlock(d)}
 <p class="eb-gt">Что нужно<span>${n}</span></p>
 ${d.rows.length?rowsList(d.rows,'proddel:'+d.id):''}
 ${addBtn('Строка','prodrow:'+d.id)}
 </div></details>`;}
function prodSection(){return `<section class="eb-sec"><div class="eb-sec-head"><h2>Райдеры события</h2>${addBtn('Свой райдер','addprod')}</div><p class="caption">Не привязаны к артистам: общий звук, свет, декор и любые свои. Уходят подрядчикам вместе с райдерами артистов.</p><div class="eb-prods">${(E.prod||[]).map(prodCard).join('')}</div></section>`;}
const ensureRider=p=>p.rider||(p.rider={src:'manual',f:'',rows:[],h:[],n:'',prov:{}});
function mergeParsed(p,res){const r=ensureRider(p);res.tech.forEach(([k,l,q])=>{const ex=r.rows.find(x=>x.k===k);if(ex)ex.q=Math.max(ex.q,q);else r.rows.push(...toRows([[k,l,q]]));});res.hosp.forEach(h=>{if(!r.h.includes(h))r.h.push(h);});const extra=[...res.notes,...res.unknown].join(' · ');if(extra)r.n=[r.n,extra].filter(Boolean).join(' · ');if(r.src==='manual'||!r.src)r.src='text';p.riderAsked='';return r;}
...
function bMoney(){return `<button type="button" class="card eb-tlink" data-cmd="goto:5"><span class="eb-need-t"><b>Билеты</b><small>${E.ticketMode==='reg'?'вход бесплатный':E.price?`${E.tickets.length} ${plural(E.tickets.length,'тип','типа','типов')} · средний ${rub(E.price)} с учётом скидок`:'цены не заданы'}</small></span><span class="eb-chev">→</span></button>
...
function bReview(){const sl=shortlist();const items=STEPS.slice(0,8).map((s,i)=>[s,stepDone(i),i]);
 return `<ul class="card eb-review">${items.map(([s,ok,i])=>`<li><a href="#build/${i}"><span class="eb-ck${ok?' ok':''}" aria-hidden="true">${ok?'✓':''}</span><span><b>${s}</b><small>${[E.date?`${dm(E.date)}, ${E.start}–${E.end} · ${E.city}`:'дата не выбрана',venue()?`${venue().name} · ${cap()} мест`:'не выбрана',`${list('artist').length} ${plural(list('artist').length,'артист','артиста','артистов')} · ${list('artist').filter(p=>p.rider).length} из ${list('artist').length} райдеров`,`${list('crew').length+list('vendor').length} в команде`,`${reqs().filter(r=>r.st!=='draft').length} из ${reqs().length} запросов отправлено`,E.ticketMode==='reg'?'вход бесплатный':E.price?`${E.tickets.length} ${plural(E.tickets.length,'тип','типа','типов')} · средний ${rub(E.price)}${E.codes.length?' · '+E.codes.length+' промокод.':''}`:'цены не заданы',`расходы ${rub(cost().total)}${E.ticketMode==='sale'&&breakEven(false)!==null?' · окупаемость '+breakEven(false)+' бил.':''}`,`${(E.channels||[]).length} ${plural((E.channels||[]).length,'канал','канала','каналов')} · ${rub(promoBudget())} · ${(E.posts||[]).length} публикаций`][i]}</small></span><span class="eb-chev">→</span></a></li>`).join('')}</ul>
...
/* ---- площадки: вместимость и свободные даты ---- */
let vf={cap:null,free:true};
const isoD=d=>d?iso(d):'';
const ownBusy=(x,d)=>EVENTS.find(ev=>ev!==E&&ev.date&&isoD(ev.date)===isoD(d)&&ev.people.some(p=>p.group==='venue'&&p.name===x.name));
const vFree=(x,d)=>!d||(!(x.busy||[]).includes(isoD(d))&&!ownBusy(x,d));
function vNear(x,d){if(!d)return [];const out=[];for(let k=1;k<=14&&out.length<3;k++)for(const sg of [-1,1]){const dd=new Date(d.getTime()+sg*k*864e5);if(out.length<3&&vFree(x,dd)&&[5,6].includes(dd.getDay()))out.push(dd);}return out.sort((a,b)=>a-b);}
function venueList(q=''){const need=vf.cap??(E.guests||0),d=E.date,cur=venue()?.name;
 let rows=CAT.venue.filter(x=>(x.name+' '+x.role).toLowerCase().includes(q.toLowerCase())).map(x=>({...x,fit:!need||x.cap>=need,free:vFree(x,d)}));
 const hidden=rows.filter(x=>!x.fit||(vf.free&&!x.free)).length;if(vf.free)rows=rows.filter(x=>x.free||x.name===cur);rows=rows.filter(x=>x.fit||x.name===cur);
 rows.sort((a,b)=>(b.free-a.free)||(Math.abs(a.cap-need*1.1)-Math.abs(b.cap-need*1.1)));
 const capOpts=[0,200,300,500,1000];
 return `<div class="eb-vf"><div class="eb-chips">${capOpts.map(c=>`<button type="button" class="chip" data-vcap="${c}" aria-pressed="${(vf.cap??need)===c||(c===0&&!(vf.cap??need))}">${c?'от '+c:'Любая'}</button>`).join('')}${need&&!capOpts.includes(need)?`<button type="button" class="chip" data-vcap="${need}" aria-pressed="${(vf.cap??need)===need}">от ${need}</button>`:''}</div>
 ${d?`<label class="eb-toggle"><input type="checkbox" data-vfree${vf.free?' checked':''}> Только свободные ${dm(d)}</label>`:'<p class="caption">Выбери дату в «Основе» — покажем, кто свободен.</p>'}</div>
 <ul class="card eb-pick eb-vlist">${rows.map(x=>{const on=x.name===cur,near=x.free?[]:vNear(x,d),over=x.cap>need*1.8&&need;return `<li>${ava(x)}<span class="eb-pick-t"><b>${esc(x.name)}</b><small>${esc(x.role)} · ${x.cap} мест · ${rub(x.fee)}${x.outdoor?' · открытая':''}</small><span class="eb-vst">${!d?'':x.free?`<em class="ok">свободна ${dm(d)}</em>`:`<em class="bad">занята ${dm(d)}${ownBusy(x,d)?' — ваше «'+esc(ownBusy(x,d).name)+'»':''}</em>${near.length?` · свободна ${near.map(z=>z.getDate()+' '+MON[z.getMonth()].slice(0,3)).join(', ')}`:''}`}${over?` <em class="warn">зал в ${(x.cap/need).toFixed(1).replace('.',',')} раза больше гостей</em>`:''}</span></span><button type="button" class="eb-mini${on?' on':''}" data-cmd="${on?'noop':'take:venue:'+encodeURIComponent(x.name)}"${on?' aria-disabled="true"':''}>${on?'Выбрана':'Выбрать'}</button></li>`;}).join('')||'<li class="eb-pick-empty">Под условия никто не подходит.</li>'}</ul>
 ${hidden?`<p class="caption">Скрыто ${hidden}: ${vf.free&&d?'заняты в дату или ':''}меньше ${need} мест.</p>`:''}`;}

/* ---- анализ даты: день недели, конкуренты, погода, праздники, зарплата, свои события ---- */
const CITY_EVENTS={'2026-10-24':[['Осенний техно-фестиваль · 5 000 гостей','big'],['Джазовый вечер в филармонии','small']],'2026-10-31':[['Хэллоуин-вечеринки во всех клубах','big']],'2026-10-17':[['Концерт хедлайнера на стадионе','big']],'2026-11-07':[['Открытие зимнего сезона клубов','small']],'2026-10-10':[['Рэп-фестиваль','small']]};
const HOLI={'2026-10-31':['Хэллоуин','+'],'2026-11-04':['День народного единства · выходной','+'],'2026-11-03':['Перед длинными выходными','+']};
function weather(d){const k=d.getDate()*7+d.getMonth()*13,t=Math.round(({8:14,9:7,10:1}[d.getMonth()]??5)+(k%7)-3),rain=[20,40,65,30,80,15,50][k%7];return {t,rain};}
function dateScore(d){if(!d)return null;const f=[],wd=d.getDay(),key=isoD(d);
 const dw={5:[18,'Пятница — сильный день'],6:[22,'Суббота — лучший день недели'],4:[4,'Четверг — слабее выходных'],0:[-6,'Воскресенье — гости уходят рано'],1:[-18,'Понедельник'],2:[-16,'Вторник'],3:[-10,'Среда']}[wd];f.push(['День недели',dw[0],dw[1]]);
 const comp=CITY_EVENTS[key]||[];if(comp.length)comp.forEach(([n,sz])=>f.push(['Конкуренты',sz==='big'?-18:-6,n]));else f.push(['Конкуренты',6,'Крупных событий в городе нет']);
 const own=EVENTS.filter(ev=>ev!==E&&ev.date&&isoD(ev.date)===key);if(own.length)f.push(['Свои события',-20,'В этот день уже '+own.map(ev=>ev.name).join(', ')]);
 const w=weather(d),out=venue()?CAT.venue.find(x=>x.name===venue().name)?.outdoor:false;f.push(['Погода',w.rain>=60?(out?-16:-4):w.t<3&&out?-8:3,`${w.t>0?'+':''}${w.t}°, осадки ${w.rain}%${out?' · открытая площадка':''}`]);
 if(HOLI[key])f.push(['Праздник',8,HOLI[key][0]]);
 const dom=d.getDate();if([5,6,7,20,21,22].includes(dom))f.push(['Зарплата',5,'Рядом с днём зарплаты — легче покупают']);else if([1,2,3,17,18,19].includes(dom))f.push(['Зарплата',-3,'Перед зарплатой — меньше спонтанных покупок']);
 if((d.getMonth()===9&&dom>=26)||(d.getMonth()===10&&dom<=3))f.push(['Каникулы',-4,'Школьные каникулы — часть аудитории уезжает']);
 const days=Math.round((d-TODAY)/864e5);f.push(['Срок подготовки',days<21?-14:days<35?-4:4,days<21?`Всего ${days} дней — мало на продажи`:`${days} дней до события`]);
 const score=Math.max(5,Math.min(98,Math.round(55+f.reduce((a,x)=>a+x[1],0)*.8)));return {score,f,w};}
function scoreWord(n){return n>=75?['отличная','ok']:n>=55?['нормальная','work']:['рискованная','bad'];}
function altDates(d){if(!d)return [];const out=[];for(let k=-14;k<=14;k++){if(!k)continue;const dd=new Date(d.getTime()+k*864e5);if(dd<=TODAY)continue;const sc=dateScore(dd);out.push([dd,sc.score]);}return out.sort((a,b)=>b[1]-a[1]).slice(0,3).sort((a,b)=>a[0]-b[0]);}
function dateCard(){const d=E.date;if(!d)return '';const a=dateScore(d),[w,c]=scoreWord(a.score);
 return `<button type="button" class="card eb-dscore" data-cmd="dateinfo"><span class="eb-dscore-n ${c}">${a.score}</span><span class="eb-need-t"><b>Дата ${w}</b><small>${a.f.filter(x=>x[1]<0).slice(0,2).map(x=>x[2]).join(' · ')||'Сильных минусов нет'}</small></span><span class="eb-mini">Анализ</span></button>`;}
function dateSheet(){const d=E.date,a=dateScore(d),[w,c]=scoreWord(a.score),alt=altDates(d);
 return `<p class="caption eb-sh-cap">Анализ даты</p><h2 id="eb-sh-t">${dm(d)}, ${WD[d.getDay()]}</h2><div class="eb-dscore-big"><span class="eb-dscore-n ${c}">${a.score}</span><div><b>Дата ${w}</b><small>из 100 · считаем по ${a.f.length} факторам</small></div></div>
 <ul class="card eb-needs eb-factors">${a.f.map(([k,v,t])=>`<li><div><span class="eb-need-t"><b>${k}</b><small>${esc(t)}</small></span><span class="eb-fv ${v>0?'ok':v<0?'bad':''}">${v>0?'+':''}${v}</span></div></li>`).join('')}</ul>
 <p class="eb-gt">Даты лучше рядом</p><ul class="card eb-needs">${alt.map(([dd,sc])=>{const [ww,cc]=scoreWord(sc),v=venue(),vx=v&&CAT.venue.find(x=>x.name===v.name);return `<li><div><span class="eb-need-t"><b>${dm(dd)}, ${WD[dd.getDay()]} <em class="eb-fv ${cc}">${sc}</em></b><small>${ww}${vx?(vFree(vx,dd)?` · ${esc(v.name)} свободна`:` · <span class="eb-hot">${esc(v.name)} занята</span>`):''}</small></span><button type="button" class="eb-mini" data-cmd="movedate:${isoD(dd)}">Перенести</button></div></li>`;}).join('')}</ul>
 <p class="caption">Погода — прогноз-заглушка, конкуренты — демо-афиша. В продукте: погодный сервис, афиша города и продажи прошлых событий Digital Jazz.</p>`;}

/* ---- каталог: список с поиском ---- */
function pickList(group,role,q=''){if(group==='venue')return venueList(q);const have=new Set(E.people.map(p=>p.name));const rows=CAT[group].filter(x=>(!role||x.role===role)&&(x.name+' '+x.role).toLowerCase().includes(q.toLowerCase()));
...
function gate(){const v=venue(),a=list('artist'),aok=a.filter(p=>p.stage>=1).length;
 const items=[[!!E.cover&&!!E.name,'Обложка и название',E.cover?'готово':'обложки нет','goto:0','Выбрать'],[!!E.date,'Дата',E.date?`${dm(E.date)}, ${E.start}`:'не выбрана','goto:0','Выбрать'],
  ...
  [E.ticketMode==='sale'&&E.price>0,'Билеты',E.ticketMode==='reg'?'сейчас вход бесплатный':E.price?`${E.tickets.length} ${plural(E.tickets.length,'тип','типа','типов')} · средний ${rub(E.price)}`:'цены не заданы','goto:5','Настроить']];
...
function summary(){const c=cost();return `${gate()}${E.date&&E.date>TODAY&&phase()<2?dateCard():''}${actions()}
 <section class="eb-sec"><div class="eb-sec-head"><h2>Окупаемость</h2><a class="text-button" href="#money">Деньги →</a></div><div class="card eb-be">${beBar()}<dl class="eb-kv"><div><dt>Расходы</dt><dd>${rub(c.total)}</dd></div><div><dt>Оплачено</dt><dd>${rub(paidSum())}</dd></div><div><dt>Средний билет</dt><dd>${E.price?rub(E.price):'—'}</dd></div></dl></div></section>
...
function nextStep(p){const r=riderSt(p);
 ...
 if(r&&r[0]!=='ok')return [p.group==='artist'?'Райдер: '+r[1]:'ТЗ: '+r[1],p.group==='artist'?'Райдер':'Запросы',p.group==='artist'?'openrider:'+p.id:'tab:req',''];
...
function riderRow(p){const r=p.rider;if(r)r.t=r.rows;return `<li class="eb-p${r?'':' wait'}"><button type="button" class="eb-p-main" data-cmd="${r?'rider:'+p.id:'parse:'+p.id}">${ava(p)}<span class="eb-p-t"><b>${esc(p.name)}</b><small>${r?`${esc(r.f||'Райдер')} · ${r.t.length} ${plural(r.t.length,'позиция','позиции','позиций')} техники · ${r.h.length} бытовых`:'Райдера нет'}</small><span class="eb-p-s"><em class="${r?'':'eb-hot'}">${r?{profile:'из профиля Digital Jazz',text:'разобран из текста',file:'загружен артистом'}[r.src]:p.riderAsked?'запрошен '+p.riderAsked+' · не прислан':'не запрошен'}</em></span></span></button>${r?'':`<span class="eb-p-r"><button type="button" class="eb-mini hot" data-cmd="askrider:${p.id}">${p.riderAsked?'Напомнить':'Запросить'}</button><button type="button" class="eb-mini" data-cmd="parse:${p.id}">Вставить текст</button></span>`}</li>`;}
function requests(){const A=list('artist'),N=needs(),H=hospNeeds(),R=reqs(),got=A.filter(p=>p.rider).length,noProv=N.filter(n=>!n.prov).length+H.filter(h=>!h.prov).length;
 ...
 const toLu=route().v==='build'?'goto:2':'tab:lineup';
 return `<button type="button" class="card eb-tlink" data-cmd="${toLu}"><span class="eb-need-t"><b>Райдеры — в лайн-апе</b><small>${A.length?`${got} из ${A.length} получено${got<A.length?' · нет: '+A.filter(p=>!p.rider).map(p=>esc(p.name)).join(', '):''}`:'артистов пока нет'}</small></span><span class="eb-chev">→</span></button>
 ...
 <p class="eb-gt">Техника<span>${N.length}</span></p><ul class="card eb-needs">${N.sort((a,b)=>(a.prod?1:0)-(b.prod?1:0)).map(n=>`<li><button type="button" data-cmd="need:${n.key}"><span class="eb-need-t"><b>${esc(n.label)} <em>× ${n.qty}</em></b><small>${n.prod?'':n.cat+' · '}${Object.keys(n.by).length>1?Object.entries(n.by).map(([sc,q])=>`${esc(sc)} ${q}`).join(' + ')+' · ':''}${n.src.join(', ')}${n.fromVenue&&n.prov!==venue()?.name?` · ${n.fromVenue} есть на площадке`:''}</small></span>${chip(n)}</button></li>`).join('')}</ul>
...
/* ---- билеты: типы, промокоды, цена по формуле ---- */
function tixSummary(){const c=cap(),be=breakEven(false),full=E.tickets.reduce((a,t)=>a+(t.qty||0)*tPrice(t),0)-discLoss(),res=full+income(false)-cost().total;
 return `<div><span class="caption">Средний билет</span><b>${E.price?rub(E.price):'—'}</b><small>с учётом скидок</small></div><div><span class="caption">Окупаемость</span><b>${be===null?'—':be+' '+plural(be,'билет','билета','билетов')}</b>${c&&be>c?`<small class="eb-hot">больше, чем мест (${c})</small>`:c&&be!==null?`<small>${Math.round(be/c*100)}% зала</small>`:''}</div><div><span class="caption">Если продать все квоты</span><b>${rub(full)}</b></div><div><span class="caption">Итог при солдауте</span><b class="${res>=0?'eb-ok':'eb-hot'}">${res>=0?'+':'−'}${rub(Math.abs(res))}</b></div>`;}
function fxLine(){const F=E.formula,w=fW();if(!w)return 'Нужны квоты или площадка — иначе не на что делить.';return `(${rub(cost().total)} − ${rub(income(false))}) × ${(1+(F.margin||0)/100).toFixed(2).replace('.',',')} ÷ (${Math.round(w)} × ${F.fill||100}%) = <b>${rub(Math.round(fBase()))}</b> базовая`;}
function tickets(){if(E.tickets.length===1&&!E.tickets[0].qty&&cap())E.tickets[0].qty=cap();const F=E.formula,q=tixQty(),c=cap(),sb=(E.sales||[]).length?soldBy():null;
 const seg=`<div class="seg eb-seg2">${[['sale','Продажа билетов'],['reg','Бесплатно']].map(([k,l])=>`<button type="button" data-set="ticketMode:${k}" aria-pressed="${E.ticketMode===k}">${l}</button>`).join('')}</div>`;
 if(E.ticketMode==='reg')return `${seg}<div class="card eb-empty"><p class="secondary">Вход по регистрации${c?`, лимит — ${c} мест по вместимости`:''}. Расходы ${rub(cost().total)} билетами не окупятся — только партнёрами.</p></div>`;
 return `${seg}<div class="card eb-calc" data-calc="tix">${tixSummary()}</div>
 <section class="eb-sec"><div class="eb-sec-head"><h2>Типы билетов</h2><span class="caption${c&&q>c?' eb-hot':''}">${q} из ${c||'?'} мест</span></div>
 <ul class="eb-tix">${E.tickets.map((t,i)=>`<li class="card eb-tk"><div class="eb-tk-h"><input class="input eb-tk-n" data-bind="t:${t.id}.n" value="${esc(t.n)}" placeholder="Название, например «Ранний»" aria-label="Название билета"><button type="button" class="eb-x" data-cmd="deltix:${t.id}" aria-label="Удалить тип">×</button></div>
 <div class="eb-tk-f"><label><span class="caption">Сколько</span><input class="input" inputmode="numeric" data-bind="t:${t.id}.qty" data-rerender value="${t.qty||''}" placeholder="0"></label>${F.on?`<label><span class="caption">Коэффициент</span><input class="input" inputmode="decimal" data-bind="t:${t.id}.k" data-rerender value="${String(t.k||1).replace('.',',')}"></label><div><span class="caption">Цена</span><b class="eb-fx-p">${rub(tPrice(t))}</b></div>`:`<label><span class="caption">Цена, ₽</span><input class="input" inputmode="numeric" data-bind="t:${t.id}.price" data-rerender value="${t.price||''}" placeholder="0"></label><div><span class="caption">Выручка</span><b>${rub((t.qty||0)*(t.price||0))}</b></div>`}</div>
 <input class="input eb-tk-d" data-bind="t:${t.id}.desc" value="${esc(t.desc||'')}" placeholder="Описание: что входит, до какой даты" aria-label="Описание билета">
 ${sb&&t.qty?`<div class="eb-tk-s"><i style="width:${Math.min(100,sb[i]/t.qty*100)}%"></i></div><small class="eb-tk-sl">${sb[i]>=t.qty?'распродан':`продано ${sb[i]} из ${t.qty}`}</small>`:''}</li>`).join('')}</ul>
 ${c&&q>c?`<p class="eb-warn">Квот больше, чем мест: ${q} при вместимости ${c}.</p>`:c&&q<c?`<p class="caption">Ещё ${c-q} ${plural(c-q,'место','места','мест')} без квоты.</p>`:''}
 ${addBtn('Тип билета','addtix')}</section>
 <section class="eb-sec"><div class="eb-sec-head"><h2>Промокоды на скидку</h2>${addBtn('Промокод','addcode')}</div>
 ${E.codes.length?`<ul class="card eb-edit eb-codes">${E.codes.map(k=>`<li><input class="input eb-code" data-bind="cd:${k.id}.code" value="${esc(k.code)}" placeholder="КОД" aria-label="Промокод"><label class="eb-sfx"><input class="input" inputmode="numeric" data-bind="cd:${k.id}.pct" data-rerender value="${k.pct||''}" placeholder="10" aria-label="Скидка, процентов"><span>%</span></label><label class="eb-sfx"><input class="input" inputmode="numeric" data-bind="cd:${k.id}.limit" data-rerender value="${k.limit||''}" placeholder="∞" aria-label="Сколько раз можно применить"><span>шт</span></label><button type="button" class="eb-x" data-cmd="delcode:${k.id}" aria-label="Удалить промокод">×</button></li>`).join('')}</ul><p class="caption">Скидки до ${rub(discLoss())} уже учтены в средней цене и окупаемости. Код без лимита считаем на 10% мест. Коды каналов в «Промо» считают продажи, эти — дают скидку.</p>`:'<p class="caption">Код, процент скидки и сколько раз его можно применить.</p>'}</section>
 <section class="eb-sec"><div class="eb-sec-head"><h2>Цена по формуле</h2><button type="button" class="eb-tog" data-cmd="formula" aria-pressed="${F.on}" aria-label="Считать цену по формуле"><i></i></button></div>
 <div class="card eb-fx${F.on?' on':''}"><p class="eb-fx-f">Базовая = (расходы − подтверждённые партнёры) × (1 + наценка) ÷ (квоты × коэффициенты × заполняемость)</p><p class="secondary">Цена типа — базовая × его коэффициент, вверх до 100 ₽. Если каждую квоту продать на заданный процент, вечер окупится и даст наценку. ${F.on?'Цены пересчитываются сами при любом изменении сметы.':'Сейчас цены задаются вручную — включи, чтобы считать от сметы.'}</p>
 <div class="eb-two"><label class="field"><span>Заполняемость, %</span><input class="input" inputmode="numeric" data-bind="F.fill" data-rerender value="${F.fill}"></label><label class="field"><span>Наценка, %</span><input class="input" inputmode="numeric" data-bind="F.margin" data-rerender value="${F.margin}"></label></div>
 <p class="eb-fx-n">${fxLine()}</p></div></section>`;}
...
function money(){const c=cost(),res=sold*E.price+income(false)-c.total;
 ...
 <section class="eb-sec"><div class="eb-sec-head"><h2>Билеты</h2><a class="text-button" href="#tickets">Настроить →</a></div>${E.ticketMode==='sale'?`<div class="card eb-tickets"><div><span class="caption">Типов</span><b>${E.tickets.length}</b></div><div><span class="caption">Средний</span><b>${E.price?rub(E.price):'—'}</b></div><div><span class="caption">Мест</span><b>${cap()||'—'}</b></div></div>`:`<p class="eb-warn">Вход бесплатный: смета ${rub(c.total)} не окупится билетами.</p>`}</section>
...
function sheetHTML(){if(!sheet)return '';const s=sheet;let h='';
 if(s.type==='person'){const p=E.people.find(x=>x.id===s.id);if(!p)return '';const st=stageOf(p);
  ...
  ${p.group==='artist'?`<div class="eb-sh-row"><span class="caption">Райдер</span>${p.rider?`<span>${p.rider.rows.filter(x=>(x.l||'').trim()).map(x=>esc(x.l)+' × '+x.q).join(', ')||'пусто'}</span>`:`<span class="eb-hot">не получен</span>`}<button type="button" class="text-button" data-cmd="openrider:${p.id}">Открыть в лайн-апе →</button></div>`:''}
 ...
 if(s.type==='rider'){const p=E.people.find(x=>x.id===s.id),r=p.rider;h=`<p class="caption eb-sh-cap">Райдер · ${{profile:'из профиля Digital Jazz',text:'разобран из текста',file:'загружен артистом'}[r.src]}</p><div class="eb-sh-head">${ava(p,'lg')}<div><h2 id="eb-sh-t">${esc(p.name)}</h2><p class="secondary">${esc(r.f||'')}</p></div></div><p class="eb-gt">Техника</p><ul class="card eb-needs">${r.rows.map(x=>`<li><div><span class="eb-need-t"><b>${esc(x.l)} <em>× ${x.q}</em></b><small>${x.cat}</small></span></div></li>`).join('')}</ul><p class="eb-gt">Бытовое</p><ul class="card eb-needs">${r.h.map(x=>`<li><div><span class="eb-need-t"><b>${esc(x)}</b></span></div></li>`).join('')}</ul>${r.n?`<p class="eb-sh-state">${esc(r.n)}</p>`:''}<button type="button" class="secondary-button eb-btn eb-full" data-cmd="parse:${p.id}">Заменить текстом из письма</button>`;}
 ...
 if(s.type==='findv'){const d=E.prod.find(x=>x.id===s.id),rows=d.rows.filter(r=>(r.l||'').trim()&&r.q),L=vFind(d),sent=new Set((d.quotes||[]).map(q=>q.name));
  if(!s.sel)s.sel=new Set(L.filter(x=>x.m.full===x.m.n&&!sent.has(x.v.name)).slice(0,3).map(x=>x.v.name));s.vo=s.vo||new Set();
  h=`<p class="caption eb-sh-cap">Подбор по райдеру</p><h2 id="eb-sh-t">${esc(d.n||'Свой райдер')}</h2>${rows.length?`<p class="secondary">${rows.length} ${plural(rows.length,'позиция','позиции','позиций')}. Отметь, у кого запросить расчёт, — сравним ответы и выберем.</p>
  ${L.length?`<ul class="card eb-vfind">${L.map(({v,m})=>{const inTeam=E.people.some(p=>p.name===v.name),ok=m.full===m.n,was=sent.has(v.name),C=vCalc(v.name,rows);return `<li><div class="eb-vq-h"><label class="eb-vq-c"><input type="checkbox" data-vsel="${esc(v.name)}"${s.sel.has(v.name)?' checked':''}${was?' disabled':''} aria-label="Запросить расчёт у ${esc(v.name)}"></label><span class="eb-pick-t"><b>${esc(v.name)}</b><small><em class="eb-cov${ok?' ok':''}">${m.full} из ${m.n}</em>${inTeam?' · уже в команде':''}${was?' · <span class="eb-rq wait">запрос отправлен</span>':''}</small></span><b class="eb-vq-sum">≈ ${rub(m.cost)}</b></div>
  <details class="eb-vcalc" data-vc="${esc(v.name)}"${s.vo.has(v.name)?' open':''}><summary>Расчёт по позициям${m.miss.length||m.short.length?` · <span class="eb-hot">${m.miss.length+m.short.length} не закрыто</span>`:''}</summary><table><tbody>${C.map(c=>`<tr class="${c.st}"><td>${esc(c.l)}</td><td>${c.st==='none'?'нет':c.have+(c.st==='short'?' из '+c.q:'')+' шт'}</td><td>${c.pr?rub(c.pr):''}</td><td>${c.sum?rub(c.sum):'—'}</td></tr>`).join('')}</tbody><tfoot><tr><td colspan="3">Итого по прайсу</td><td>${rub(m.cost)}</td></tr></tfoot></table><button type="button" class="text-button" data-cmd="vpick:${d.id}:${encodeURIComponent(v.name)}:${m.cost}">Выбрать сразу, без запроса</button></details></li>`;}).join('')}</ul>
  <p class="caption">Оценка — по прайсу склада. В запрос уйдут позиции, количество, дата ${E.date?dm(E.date):''}, площадка ${esc(venue()?.name||'')} и время монтажа.</p>
  <button type="button" class="primary eb-btn eb-full" data-cmd="vsend:${d.id}"${s.sel.size?'':' disabled'}>${s.sel.size?`Отправить запрос · ${s.sel.size}`:'Отметь подрядчиков'}</button>`:'<div class="card eb-empty"><p class="secondary">В каталоге никто не закрывает эти позиции. Выбери подрядчика вручную или добавь своего в «Команде».</p></div>'}`:'<div class="card eb-empty"><p class="secondary">Сначала заполни райдер — подбор идёт по его строкам.</p></div>'}`;}
...
function swipeInit(){document.querySelectorAll('.eb-bsteps,.eb-chips:not(.wrap)').forEach((n,i)=>{let w=n.parentElement;
 if(!w.classList.contains('eb-swipe')){w=document.createElement('div');w.className='eb-swipe'+(n.classList.contains('eb-bsteps')?' is-steps':'');n.before(w);w.append(n);w.insertAdjacentHTML('beforeend',ARR(-1)+ARR(1));}
...
document.addEventListener('click',e=>{const c=e.target.closest('[data-vcap]');if(!c)return;vf.cap=+c.dataset.vcap;const host=document.querySelector('[data-pick-host]');if(host&&sheet)host.innerHTML=venueList(sheet.q||'');else render();});
document.addEventListener('change',e=>{if(!e.target.matches('[data-vfree]'))return;vf.free=e.target.checked;const host=document.querySelector('[data-pick-host]');if(host&&sheet)host.innerHTML=venueList(sheet.q||'');else render();});
...
function render(){if(window.EB_HEADLESS)return;const r=route();let html;
 if(r.v==='events')html=events();
 else if(r.v==='build')html=build(r.step);
 else html=header()+`<div class="eb-body">${r.v==='summary'?summary():r.v==='place'?place():r.v==='people'?people():r.v==='promo'?promo():r.v==='req'?requests():r.v==='money'?money():r.v==='lineup'?bLineup():r.v==='tickets'?tickets():day()}</div>`;
...
function target(ref){const [scope,key]=ref.split('.');if(scope==='E')return [E,key];const [k,id]=scope.split(':');if(scope==='F')return [E.formula,key];if(k==='rh'){const p=E.people.find(x=>x.id===id);return [p&&p.rider&&p.rider.h,key];}const arr={p:E.people,c:E.costs,i:E.incomes,s:E.stages,ch:E.channels,po:E.posts,t:E.tickets,cd:E.codes,rr:[...E.people.flatMap(p=>p.rider?p.rider.rows:[]),...(E.prod||[]).flatMap(d=>d.rows)],pr:E.prod||[]}[k];return [arr.find(x=>x.id===id),key];}
document.addEventListener('input',e=>{const t=e.target;t.closest?.('.eb-dtf')?.classList.toggle('is-empty',!t.value);
 if(t.dataset.bind){const [o,k]=target(t.dataset.bind);if(!o)return;o[k]=k==='date'||(k==='d'&&t.type==='date')?(t.value?new Date(t.value+'T00:00'):null):['fee','cap','price','a','paid','budget','guests','len','qty','q','pct','limit','fill','margin'].includes(k)?num(t.value):k==='k'?(parseFloat(t.value.replace(',','.'))||0):k==='code'?t.value.toUpperCase().replace(/\s+/g,''):t.value;
  if(t.dataset.bind.startsWith('rr:')&&k==='l')o.k='';
  if(k==='pct')o.pct=Math.min(100,o.pct);
  {const tk=t.closest('.eb-tk');if(tk&&!E.formula.on){const b=tk.querySelector('.eb-tk-f div b');if(b)b.textContent=rub((o.qty||0)*(o.price||0));}}
  if(k==='date')E.announce=E.date?new Date(E.date.getTime()-21*864e5):null;
  document.querySelectorAll('[data-calc=summary]').forEach(n=>n.innerHTML=calcSummary());document.querySelectorAll('[data-calc=tix]').forEach(n=>n.innerHTML=tixSummary());document.querySelectorAll('[data-calc=promo]').forEach(n=>n.textContent=rub(promoBudget()));document.querySelectorAll('[data-calc=costsum]').forEach(n=>n.textContent=rub(E.costs.reduce((a,c)=>a+c.a,0)));return;}
 if(t.dataset.rdnote){const p=E.people.find(x=>x.id===t.dataset.rdnote);ensureRider(p).n=t.value;return;}
...
document.addEventListener('change',e=>{const n=e.target.dataset?.vsel;if(n===undefined||!sheet)return;e.target.checked?sheet.sel.add(n):sheet.sel.delete(n);render();});
document.addEventListener('toggle',e=>{const d=e.target;if(d.dataset&&d.dataset.vc&&sheet&&sheet.vo){d.open?sheet.vo.add(d.dataset.vc):sheet.vo.delete(d.dataset.vc);return;}if(d.dataset&&d.dataset.rd)d.open?openR.add(d.dataset.rd):openR.delete(d.dataset.rd);},true);
const SAMPLE={art:'Технический райдер\nCDJ-3000 x2\nDJM-900NXS2 1\nСценический монитор 2\nВода без газа 4 бут\nЗакрываемая гримёрная\nСаундчек 30 минут, наушники привозит артист',Звук:'Порталы L-Acoustics 4\nСабвуферы 4\nПульт FOH 1\nМониторная линия 2',Свет:'Световые головы 8\nСтробоскоп 2\nХейзер 1\nСветовой пульт 1',Декорации:'Бэкдроп 6×3 м 1\nНеоновая вывеска 1\nФермы для декора 2',x:'Позиция из ТЗ 1'};
document.addEventListener('change',e=>{const fi=e.target.dataset?.rdfile;if(fi&&e.target.files[0]){const file=e.target.files[0],cmd=e.target.dataset.parse,p=E.people.find(x=>x.id===fi);if(p)ensureRider(p);filesOf(fi).push({name:file.name});openR.add(fi);
  const go=tx=>{render();const ta=document.querySelector(`[data-rd="${fi}"] textarea[data-rdtext],[data-rd="${fi}"] textarea[data-prtext]`);if(ta){ta.value=tx;document.querySelector(`[data-cmd="${cmd}:${fi}"]`)?.click();}};
  if(/\.txt$/i.test(file.name)){const fr=new FileReader();fr.onload=()=>go(String(fr.result));fr.readAsText(file);}else{const d=(E.prod||[]).find(x=>x.id===fi);go(p?SAMPLE.art:SAMPLE[d&&d.cat]||SAMPLE.x);}return;}
 const pp=e.target.dataset?.prodprov;if(pp){E.prod.find(x=>x.id===pp).prov=e.target.value;render();return;}const pv=e.target.dataset?.rdprov;if(pv){const [id,c]=pv.split(':'),p=E.people.find(x=>x.id===id);ensureRider(p).prov[c]=e.target.value;render();return;}if(e.target.matches('[data-rerender]')){clearTimeout(render.t);render.t=setTimeout(render,0);}const u=e.target.dataset?.upload;if(u&&u.startsWith('post:')&&e.target.files[0]){const p=E.posts.find(x=>x.id===u.slice(5));p.own=e.target.files[0].name;render();say('Макет загружен: '+p.own);return;}if(u&&e.target.files[0]){const n=e.target.files[0].name;E.assets[u]=u==='poster'?'file':{name:n};sheet=null;render();say('Загружено: '+n);}});
...
document.addEventListener('click',e=>{const t=e.target;
 ...
 const rc=t.closest('[data-rdcat]');if(rc){const [id,c]=rc.dataset.rdcat.split(':');riderCat[id]=c;openR.add(id);render();return;}
 ...
 switch(cmd){
  ...
  case 'tickets':E.ticketMode='sale';sheet=null;location.hash='tickets';render();return;
  ...
  case 'noop':return;
  case 'addtix':E.tickets.push({id:uid(),n:'',qty:0,price:0,k:1,desc:''});render();document.querySelector('.eb-tix li:last-child .eb-tk-n')?.focus();return;
  case 'deltix':E.tickets=E.tickets.filter(x=>x.id!==a);render();return;
  case 'addcode':E.codes.push({id:uid(),code:'',pct:10,limit:0});render();document.querySelector('.eb-codes li:last-child .eb-code')?.focus();return;
  case 'delcode':E.codes=E.codes.filter(x=>x.id!==a);render();return;
  case 'formula':{const F=E.formula;if(F.on)E.tickets.forEach(t=>{t.price=tPrice(t);});else E.tickets.forEach(t=>{if(!t.k)t.k=1;});F.on=!F.on;render();say(F.on?'Цены считаются от сметы':'Цены зафиксированы — можно править вручную');return;}
  case 'openrider':{openR.add(a);sheet=null;const h=route().v==='build'?'build/2':'lineup';if(location.hash.slice(1)===h)render();else location.hash=h;setTimeout(()=>document.querySelector(`[data-rd="${a}"]`)?.scrollIntoView({block:'start',behavior:'smooth'}),200);return;}
  case 'rdrow':{const p=P(a),c='Звук';ensureRider(p).rows.push({id:uid(),k:'',l:'',q:1,cat:c});openR.add(a);render();const L=document.querySelectorAll(`[data-rd="${a}"] .eb-rd-rows:not(.h) li`);L[L.length-1]?.querySelector('input')?.focus();return;}
  case 'rddel':{const p=P(a);p.rider.rows=p.rider.rows.filter(x=>x.id!==b);render();return;}
  case 'rdh':{const p=P(a);ensureRider(p).h.push('');openR.add(a);render();const L=document.querySelectorAll(`[data-rd="${a}"] .eb-rd-rows.h li`);L[L.length-1]?.querySelector('input')?.focus();return;}
  case 'rdhdel':{const p=P(a);p.rider.h.splice(+b,1);render();return;}
  case 'rdparse':{const p=P(a),tx=document.querySelector(`[data-rdtext="${a}"]`)?.value||'';if(!tx.trim()){say('Вставь текст райдера — разберём по строкам');return;}const res=parseRider(tx);mergeParsed(p,res);openR.add(a);render();say(`${p.name}: техника ${res.tech.length}, бытовое ${res.hosp.length}${res.notes.length+res.unknown.length?', в заметки '+(res.notes.length+res.unknown.length):''}`);return;}
  case 'vsend':{const pr=E.prod.find(x=>x.id===a),rows=pr.rows.filter(r=>(r.l||'').trim()&&r.q),names=[...sheet.sel];pr.quotes=pr.quotes||[];
   names.forEach((nm,i)=>{const est=vMatch(nm,rows).cost,q={name:nm,st:'sent',est,sum:0,at:TODAY.getDate()+' '+MON[TODAY.getMonth()].slice(0,3)};pr.quotes.push(q);
    setTimeout(()=>{q.sum=quoteSum(nm,est);q.st='quoted';render();say(`${nm} прислал расчёт: ${rub(q.sum)} · ${pr.n}`);},2600+i*1500);});
   openR.add(a);sheet=null;render();say(`Запрос расчёта ушёл: ${names.join(', ')}`);return;}
  case 'qacc':{const pr=E.prod.find(x=>x.id===a),q=pr.quotes.find(x=>x.name===B1);pr.quotes.forEach(x=>{if(x!==q&&x.st!=='accepted')x.st='closed';});q.st='accepted';const ex=E.people.find(p=>p.name===B1);if(ex)ex.fee=Math.max(ex.fee,0)+q.sum;else{const x=CAT.vendor.find(v=>v.name===B1);addPerson('vendor',{...x,fee:q.sum});}pr.prov=B1;render();say(`${B1} — подрядчик «${pr.n}» · ${rub(q.sum)} в смете, остальным ушёл отказ`);return;}
  case 'qnud':say('Напомнили: '+A1);return;
  case 'findv':open({type:'findv',id:a});return;
  case 'vpick':{const est=+d||25000,pr=E.prod.find(x=>x.id===a),nm=B1;if(!E.people.some(p=>p.name===nm)){const x=CAT.vendor.find(v=>v.name===nm);addPerson('vendor',{...x,fee:est});}pr.prov=nm;sheet=null;render();say(`${nm} — подрядчик «${pr.n}» · ${rub(est)}`);return;}
  case 'fileopen':say('Демо: здесь откроется '+filesOf(a)[+b].name);return;
  case 'filedel':filesOf(a).splice(+b,1);render();return;
  case 'addprod':{const d={id:uid(),n:'',cat:'',rows:[],note:'',prov:'',custom:true};E.prod.push(d);openR.add(d.id);render();document.querySelector(`[data-rd="${d.id}"] .eb-tk-n`)?.focus();return;}
  case 'delprod':E.prod=E.prod.filter(x=>x.id!==a);render();return;
  case 'prodrow':{const d=E.prod.find(x=>x.id===a);d.rows.push({id:uid(),k:'',l:'',q:1,cat:d.cat});openR.add(a);render();const L=document.querySelectorAll(`[data-rd="${a}"] .eb-rd-rows li`);L[L.length-1]?.querySelector('input')?.focus();return;}
  case 'proddel':{const d=E.prod.find(x=>x.id===a);d.rows=d.rows.filter(x=>x.id!==b);render();return;}
  case 'prodparse':{const d=E.prod.find(x=>x.id===a),tx=document.querySelector(`[data-prtext="${a}"]`)?.value||'';if(!tx.trim()){say('Вставь текст — разберём по строкам');return;}let k=0;const notes=[];tx.split(/\n+/).map(l=>l.trim()).filter(Boolean).forEach(line=>{if(NOTE.test(line)){notes.push(line);return;}const m=line.match(/(?:[x×х*]\s*(\d+))|(\d+)\s*(?:шт|штук|pcs)|[—–-]\s*(\d+)\s*$|\s(\d+)\s*$/i);const q=m?+(m[1]||m[2]||m[3]||m[4]):1;const dd=DICT.find(x=>x[2].test(line));const l=(dd?dd[1]:line.replace(m?m[0]:'','').replace(/[—–:-]\s*$/,'').trim())||line;const ex=d.rows.find(x=>x.l.toLowerCase()===l.toLowerCase());if(ex)ex.q=Math.max(ex.q,q);else{d.rows.push({id:uid(),k:dd?dd[0]:'',l,q,cat:d.cat});k++;}});if(notes.length)d.note=[d.note,notes.join(' · ')].filter(Boolean).join(' · ');openR.add(a);render();say(`${d.n||'Райдер'}: добавлено ${k}${notes.length?', в заметки '+notes.length:''}`);return;}
  case 'ai':say('ИИ-помощник — следующий этап: здесь откроется визард');return;
  case 'dateinfo':if(!E.date){location.hash='build/0';return;}open({type:'date'});return;
  case 'movedate':{const nd=new Date(a+'T00:00');E.date=nd;E.announce=new Date(nd.getTime()-21*864e5);E.posts=E.posts&&E.posts.length?planPosts(E):E.posts;sheet=null;render();say('Дата перенесена на '+dm(nd)+' · сроки пересчитаны');return;}
  ...
  case 'saverider':{const p=P(a),r=sheet.res;p.rider=null;mergeParsed(p,r);p.rider.f='Райдер из письма';openR.add(p.id);sheet=null;say(`Райдер ${p.name}: ${r.tech.length} техн. и ${r.hosp.length} быт. — запросы обновлены`);return;}
...
/* публичные данные для афиши: то, что организатор собрал здесь, зритель видит там */
function pubEvent(ev){const s=E;E=ev;try{const v=venue(),sb=(E.sales||[]).length?soldBy():E.tickets.map(()=>0),A=list('artist'),conf=A.filter(p=>p.stage>=1),L=conf.length?conf:A;
 return {id:ev.id,name:ev.name,type:ev.type,date:ev.date,start:ev.start,end:ev.end,city:ev.city,cover:ev.cover?'../dj-event-builder/covers/'+ev.cover+'.jpg':null,announce:ev.announce,
...
addEventListener('click',e=>{if(window.EB_HEADLESS)return;const s=e.target.closest('[data-service]');if(!s)return;const U='../digital-jazz-unified/';const to={'турниры':U+'index.html#tournaments','афиша':'../dj-afisha/index.html','магазин':U+'index.html#shop','туры':U+'tours/index.html'}[s.dataset.service];if(!to)return;e.preventDefault();e.stopImmediatePropagation();location.href=to;},true);
