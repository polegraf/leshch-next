...
function checkout(e){const n=count(e),reg=e.mode==='reg',tot=sub(e)-disc(e);if(!n){location.hash='t/'+e.id;return '';}
 ...
 <form class="af-form" data-af-pay="${e.id}"><label class="field"><span>Имя на билете</span><input class="input" name="n" required autocomplete="name" value="${esc(window.djAccess?.profiles().find(p=>p.type==='personal')?.name||'')}"></label><label class="field"><span>Email для билетов</span><input class="input" name="m" type="email" required autocomplete="email" value="${esc(window.djAccess?.email()||'')}"></label>
...
document.addEventListener('click',ev=>{const b=ev.target.closest('button,a');if(!b)return;const d=b.dataset;
 ...
 if(d.afRemind&&window.djAccess&&!djAccess.signed){ev.preventDefault();const id=d.afRemind;djAccess.require('Войди, чтобы получить напоминание',()=>document.querySelector(`[data-af-remind="${id}"]`)?.click());return;}
 ...
 const hr=b.getAttribute('href')||'';if(hr.startsWith('#c/')&&window.djAccess&&!djAccess.signed){ev.preventDefault();djAccess.require('Войди, чтобы купить билеты — они сохранятся в профиле',()=>{location.hash=hr.slice(1);});}});
document.addEventListener('dj-access',()=>{const y=scrollY;render();scrollTo(0,y);});
