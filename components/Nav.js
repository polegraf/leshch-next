        ...
        <div style={{ position: 'fixed', inset: 0, top: 56, background: '#000', zIndex: 99, display: 'flex', flexDirection: 'column', padding: '48px 20px' }}>
          {links.map(({ href, label }) => (
            <Link key={href} href={href} onClick={() => setMenuOpen(false)}
              style={{ fontSize: 48, fontWeight: 700, letterSpacing: '-.03em', lineHeight: 1.1, color: pathname === href ? '#fff' : 'rgba(255,255,255,.35)', cursor: 'pointer', marginBottom: 16, textDecoration: 'none', ...HN }}>
              {label}
            </Link>
          ))}
        </div>
